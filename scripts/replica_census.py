...
def seed(out: pathlib.Path) -> int:
    """Write the harvest as a markdown WORKING LIST.

    Deliberately a separate file from `00c-replicas.md`. That one is the REGISTER —
    confirmed sets, each with an anchor in source. This is the pile it gets reduced
    from, and mixing them would make a suspicion look like a ruling.

    Capture broadly, reduce later: everything unregistered is listed, strongest first,
    because a candidate that turns out to be nothing costs one line and a candidate
    that is never written down costs the next outage. The STATUS column is the only
    part a regeneration cannot recompute — so this file is regenerated by hand, never
    by a gate, and edits to STATUS are the point of it existing.
    """
    findings = [f for f in harvest() if not f["registered"]]
    ...
    L = [
        ...
        "",
        f"**{len(findings)} files** carry a replica notice and no `RN` anchor; "
        f"**{len(strong)}** hold a STRONG one.",
        "",
    ...
    def rows(group):
        for f in group:
            best = next((c for c in f["clusters"] if c["tier"] == "strong"), f["clusters"][0])
            q = best["quote"].replace("|", r"\|")[:150]
            L.append(f"|  | `{f['file']}`:{best['at']} | {f['strong']}/{f['notices']} | {q} |")
    rows(strong)
    L += ["", "## Weak notices only — likely prose, review cheaply", "",
          "| STATUS | File | Notices | Notice |", "|---|---|---|---|"]
    for f in [x for x in findings if not x["strong"]]:
        c = f["clusters"][0]
        q = c["quote"].replace("|", r"\|")[:150]
        L.append(f"|  | `{f['file']}`:{c['at']} | {f['notices']} | {q} |")
    L.append("")
    out.write_text("\n".join(L) + "\n", encoding="utf-8")
    print(f"seeded {out} — {len(findings)} candidates ({len(strong)} strong)")
    return 0
...
def main() -> int:
    ...
    ap.add_argument("--seed", metavar="PATH", default=None,
                    help="emit the harvest as a markdown working list")
    args = ap.parse_args()

    if args.seed:
        return seed(pathlib.Path(args.seed))
