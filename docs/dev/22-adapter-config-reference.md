...
| `message_is_code` | `bool` | `False` | `True` when this brand's `error_message` entity **state is itself the code** rather than prose about it. On a message-channel rising edge the tracker then carries that state into `code` (via `_code_key()`) whenever the attribute route above yields nothing; the attribute still wins when present. A **declaration, never a sniff**: Roborock's error entity reports `bumper_stuck`, Eufy's reports "Robot is stuck", and applying this ungated would mint `robot is stuck` as a pseudo-code and poison every table below with unmatchable keys. Omitting it keeps attribute-only capture. Read by `_read_error_code_for_message()`; closes `live:RB-ERR-2`, which left all five tables below unreachable for Roborock. |
