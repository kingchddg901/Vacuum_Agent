...
| `reset_button` | dict \| None | Resolves the component's reset button: `entity_suffixes` (appended to `button.{object_id}_`) tried first, then a **sibling suffix** match, then the sibling whose upstream **`translation_key`** IS the suffix, then `token_sets` as all-tokens-must-match fallbacks. The translation_key rung is `INR2F03P` and is what makes this work on a localized install: a German reset button is `..._hauptbursten_verbrauchsmaterial_zurucksetzen`, which contains no "reset" and shares no suffix, so every earlier rung misses — while the declared suffixes happen to be byte-identical to Roborock's upstream keys, so the rung needs no new vocabulary. A resolved-but **DISABLED** button is reported as no button at all: pressing one is a silent no-op, so offering it would be worse than omitting it. Since 2.1.0-beta.2 the token search looks at the vacuum's **device and config entry** siblings first (accepting only an unambiguous single match) before the original registry-wide `button.{object_id}_` prefix scan — see the note below. A `token_sets` match is additionally rejected if its resolved `entity_id` contains the substring `"maintenance"`. Absent → no reset button. |
...
> The two-source model is **Eufy-specific**. The `usage_hours` / `total_life_hours` sensor-attribute contract is Eufy's. On **Roborock**, the life-tracked `*_time_left` sensors are **device-owned countdowns** exposing **neither** attribute. Until 2026-08-16 that meant every replacement row resolved `total_life_hours=None → remaining_percent=None → replacement_status = "unknown"` — six items, six flagged for attention, zero healthy, with "232.3 hours remaining" printed beside the word Unknown (issue #51). **The attribute is an Eufy-ism, and the adapter already declares the life we need**, so when the attribute is absent `total_life_hours` now falls back to the component's `default_interval_hours` — the same number the MAINTENANCE half of the very same loop has always read, which is why one screen could say "300 hours left of 300 hours" while the replacement row beside it said Unknown. The attribute still wins when present, so Eufy is untouched, and a component with neither stays honestly `unknown`. The integration-maintenance row still never decrements (`usage_hours` missing → `current_usage=0.0` → `remaining=interval` always). `default_interval_hours`/`max_interval_hours` are advisory for Roborock — the device, not the framework interval, drives the real countdown, and the device resets itself when its own reset button is pressed. The former `remaining_is_state` flag (declared to gate a parallel device-countdown model, "Wave 1b") was **removed 2026-07-30** — it was `True` on only 4 of the 12 components, projected (with a `False` default) onto all 12 in the registered config, with zero readers (closes code-flag CS-2 by pruning; re-add it *with* its consumer if Wave 1b ships).
