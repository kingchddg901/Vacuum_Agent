...
runtime seam that [19 — Card Architecture](19-card-architecture.md) gestures at but never details.

Read first, then come back:
- **[19 §3.1–3.2](19-card-architecture.md)** owns the render cycle (`_scheduleRender` microtask
...
- **[19 §2.2](19-card-architecture.md)** owns the four-layer prototype-mixin pattern
  (actions / state / renderers / bindings).
- **[34 — Frontend Module Reference](34-frontend-module-reference.md)** owns the full `src/bindings/*.js`
...
from scratch is safe is [19 §3.2](19-card-architecture.md)'s invariant — not repeated here.
...
- **Theme editor** (`bindings/theme.js`): `[data-theme-token]` binds both — `"input"` for live apply
  ...
  the 600 ms debounce owned by [19 §3.1](19-card-architecture.md), so the modified-badge update
...
Every re-render funnels through `card._scheduleRender()` (microtask dedup — [19 §3.1](19-card-architecture.md)).
The `hass` setter's own refresh cascade (debounced service refreshes + load-once flags) is owned by
[19 §4.3](19-card-architecture.md) — **not restated here**. Below are the triggers that fire a render
...
Binding / modal-host specific. Anything about the render cycle itself is [19 §3.1–3.3](19-card-architecture.md).
...
- **Double-click disambiguation** (the 220 ms timer) is a render-cycle cliff, not a binding one —
  see [19 §3.3](19-card-architecture.md).
