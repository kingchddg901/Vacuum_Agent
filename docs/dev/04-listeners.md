...
3. `async_get_map_live_pose(...)` → `robot_anchor` (may be `None`). Brand-generic since v2.0.1: the adapter's `map_state_source.live_pose.backend` selects the reader ([31-map-source-coordinator](31-map-source-coordinator.md#32-async_get_map_live_pose)). Until it was, a brand that could not describe itself in eufy-clean's terms got `not_configured` here and the capture drew a room with **no dot** — the Roborock symptom this step's genericity exists to prevent.
4. A **trail** from the 24-hour pose ring (`pose_store.read_range`, §10), executor-offloaded. Samples with a null anchor are **dropped, not zeroed** — a docked or held tick is a genuine `None`-run, and coercing it would draw a line to the origin. The backward half is free (the ring already holds it) and is what separates "wedged" from "slow": unchanged anchors across the window are no-movement evidence the counters cannot provide.

   The ± window is **derived, not fixed**: `trail_window_seconds(pose_refresh_s)` sizes it so the brand's declared pose cadence yields enough distinct positions to be evidence, floored at the historical **30 s**. Eufy's ~2 s pose needs only ~5 s, so it sits on the floor and its picture is unchanged; Roborock's ~30 s map refresh derives **±75 s**. The same cadence goes to the renderer, which draws sparse samples as **breadcrumbs** rather than a line — widening the window to collect coarse samples and then joining them would just be a bigger fabrication, since at ~30 s apart the robot covers several metres and certainly turned. A brand declaring no cadence keeps ±30 s and the line.
