...
2. **`localized_bins`** — only for very large components (`area >= max(10 × min_area_pixels, 120_000)`). Quantises normalised chromaticity features into 8-level (0–7) bins, ranks bins by size (filtering to `[min_area_pixels × 0.9, active_area × 0.24]` range), selects up to 6 colour bins and 4 hue bins as seeds, and grows each seed back into the component.
...
`segmentation_mode` (`"cv"` | `"custom"`, default `"cv"`) selects which segment store `get_map_segments` serves; in `custom` mode it serves the **active** layout. **All stores persist independently** — toggling the mode (or switching layouts) is a pointer change, not a regeneration. Flipping `cv → custom → cv` returns each segment set untouched, with zero re-analysis. `_handle_get_map_segments` echoes `segmentation_mode`, `active_custom_layout_id`, the `custom_layouts` summary, and the active `segment_room_links` so the card knows exactly which store it is rendering. Beyond those, the response also carries `saved_zones`, `companion_anchors`, `hidden_regions` (map-level physical masks), `area_label_anchors` (A5-FURNIS-4: a **derived** `{room_id: {pct_x, pct_y}}` view built by `resolve_area_label_anchors`, merging the legacy map-level `area_label_anchors` side-table with each room's own `label_anchor` field — a room-record value wins where both exist; see [08-rooms-system](08-rooms-system.md) §6), `image_variants`, and `adjustments`. The per-layout `custom_layouts` summary now includes `backdrop_source` alongside `backdrop_variant`, plus the furnished-render keys `render_mode`, `home_art`, and `rooms` (Wave 0 furnished custom render).
