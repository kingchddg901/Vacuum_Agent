...
4. Call `detect_capabilities(hass, *, vacuum_entity_id, detected_model, entity_candidates, model_family, capability_hints, maintenance_components, reserved_suffixes)` (all args after `hass` are keyword-only) — augments each candidate list with device-registry siblings (live:ENT-1, §3.5), then probes the HA entity registry and state machine; returns capability flags and resolved entity IDs.
   ...
   **Competing candidates are refused, not ranked by luck (live:ENT-6).** When more than one sibling matches a role, appending them all and taking the first that exists makes entity-registry INSERTION ORDER the tiebreaker — which nobody chose, and which can bind a card to the wrong machine when two vacuums share a config entry. Two narrowings are tried (a sibling still carrying the vacuum's own object_id, then one matching the majority companion stem); if neither yields exactly one, the role stays UNRESOLVED and the competitors are recorded under `entity_augmentation.ambiguous` for the options UI to offer. This matches the discipline `resolve_declared_entities` already applied. The majority stem is computed by stripping each sibling's owning suffix and voting, and **a tie is not a majority** — a dead heat yields `None` rather than whichever stem was inserted first.

   Two diagnostics-only keys come back beside the capability flags, and **nothing branches on either**: `entity_resolution_reasons` (per role: `resolved` / `disabled` / `registered_no_state` / `absent` — **live:ENT-2**, because a bare `null` cannot distinguish "no such entity" from "present but switched off", and those need opposite fixes) and `entity_augmentation` (`ran`, `siblings_seen`, `merged`, `reason`, `error` — **live:ENT-3**, because every failure path in the sibling rescue used to return silently, making "it died" and "it found nothing" identical in a dump).
