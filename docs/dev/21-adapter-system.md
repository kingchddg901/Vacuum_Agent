...
> **Note:** `mapping` is **not** a key in `ADAPTER_CONFIG_SCHEMA` — the 17 keys
> above are the complete set. `_validate_adapter()` nonetheless validates an
> optional `mapping` block (segmenter engine + tuning) *when one is present*
> (see §2.2); it is checked opportunistically by the registry, not declared in
> the schema dict. The Eufy CV segmenter itself lives in
...
| `adapters/eufy/segmentor.py` | `detect_room_segments()` — Eufy CV map-segmentation pipeline (the brand's segmenter engine) |
