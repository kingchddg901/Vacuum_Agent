...
| coded-label | `state/coded-label.js` | **Pure resolver, not registered on the prototype.** `resolveCodedLabel(descriptor, t, options)` answers one question — "resolve a backend `{code, params, message}` to a translated sentence" — for the two vocabularies that ask it: access-graph issues (`room_access.issue.*`) and start block reasons (`rooms.block_reason.*`); callers supply only their namespace. Preference order: translated code+params → the backend's own `message` (last fallback, kept deliberately server-emitted so a code this card release has never heard of still renders instead of going silent) → never a bare key, never blank. Imported by `access-issue-label.js`. |
| access-issue-label | `state/access-issue-label.js` | **Pure resolver, not registered on the prototype.** `accessIssueLabel(issue, t)` — the access-graph-scoped wrapper over `coded-label.js`'s `resolveCodedLabel` (A6-AGX-4: every access-graph issue used to reach the user as English prose composed in Python; now the backend emits `code` + `params` and this resolves them to a translated phrase). Imported by `state/room-access.js` and `renderers/room-access.js`. |
