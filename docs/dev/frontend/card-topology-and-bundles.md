...
This doc maps the frontend's *element topology* — the three standalone Lovelace cards that ship alongside the sidebar panel, the three self-contained ESM bundles the build emits, and the `<eufy-vacuum-map>` host shim that lets the map renderer mixin live inside a standalone card. Start at [architecture-overview.md](architecture-overview.md) for the hub; the three-bundle boundary is also the CSS boundary described in [styles-system.md](styles-system.md), and every file named here has a per-file entry in [module-reference.md](module-reference.md).

Everything in [architecture-overview.md](architecture-overview.md) describes the **sidebar panel** (the monolithic `EufyVacuumCommandCenter`). The integration also ships **three standalone Lovelace cards** a user drops onto their own dashboards. They reuse the panel's *patterns and helpers* but are independent custom elements.
...
| `vacuum-agent-profile-card` | Vacuum Agent Profile Card | `vacuum_entity_id` (req), `map_id` (req), `profile_id` (req) | `src/cards/profile-card.js` |

All three follow the HA card contract (`setConfig`, `set hass`, `getConfigElement`, `getStubConfig`, `window.customCards.push`) and define themselves via `defineCard(name, cls)` — an **idempotent** `customElements.define` (the same element is defined by more than one bundle; see the bundles section below). All three cards additionally wire HA 2026.6's opt-in **`getEntitySuggestion`** hook on their `window.customCards` entries (the entity-first card picker suggests them for a picked vacuum) — the decision logic is pure and unit-tested in `src/cards/card-suggestions.js`, gated on `isManagedVacuum` (any entity carrying a matching `vacuum_entity_id` attribute) so the card is never suggested for an unmanaged vacuum; older HA ignores the field. The dashboard card is the multi-room control surface (header → map → Rooms accordion → profiles → app scenes → Start/Dock); the room card is single-room; the **profile card** is a stripped inspect-and-run surface for ONE saved run profile (name + metadata + a read-only "Runs As" step manifest + a Run button → `start_run_profile`). Their shared, DOM-light helpers live in **`src/cards/_shared.js`** (`esc`, `vocab`, `roomSwitchesFor`, `adapterOptions`, `committedRoomFields`, `chipRow`, `callResponse`, `stripNull`, `registerCard`, `defineCard`, and the language control `renderLangControl`/`wireLangControl`). The profile card reads the run-profile `library` (which carries `steps`) from `get_saved_run_profiles`, and its "Runs As" manifest is the **shared pure `renderStepsManifest`** in `src/state/steps-manifest.js` — the *same* helper the panel's run-profiles renderer calls, so the two surfaces render the step sequence byte-identically (the anti-drift seam; unit-tested in `steps-manifest.test.mjs`).

The dashboard card's **arm-then-Start** dispatch is pure + unit-tested in `src/cards/dashboard-dispatch.js` (`nextArmed` mutual-exclusivity reducer, `planStart`, `armedIsValid`). Nothing reaches the vacuum until **Start** — load-bearing because the Eufy app scene (`select.<obj>_scene`) *fires on `select_option`*, so the card only stores the choice locally and fires on Start (see [dashboard-card.md](dashboard-card.md) §6, *App scenes — Eufy-only, fires-on-select*).
...
The lazy map URL is an absolute *served* URL, marked `external: ["/eufy_vacuum/frontend/*"]` in esbuild so it's left as a runtime `import()` (the same pattern `main.js` uses for the animal-svg manifest). The heavy ~1 MB map graph loads only when `show_map` is on. Because the cards bundle and the panel bundle both define all three standalone cards (`vacuum-agent-dashboard` / `eufy-room-card` / `vacuum-agent-profile-card`), the `defineCard` guard keeps the second define a no-op.

**Cache-busting.** Two build-injected content hashes (esbuild `define`): `__ASSET_VER__` (textures dir) and `__LOCALE_VER__` (the shipped locales dir). The locale loader appends `?v=__LOCALE_VER__` to the shipped `index.json` + per-locale fetches so an edited catalog can't be served stale (the symptom was newly-added keys falling back to English while older keys rendered translated). See [i18n-system.md](i18n-system.md) for the *escaping* counterpart — `t()` output is already HTML-escaped (trust model B), so it must never be passed through `esc()`/`escapeHtml()` again.
...
**Per-context, per-device view state.** Three map prefs are persisted in `localStorage`, all keyed so the panel and an embedded card never fight (the third is the moved **m² area chips** — key `evcc_map_room_areas_<vac>_<mapId>`, the same `_labelStorageKey`/`_setLabelAnchor` implementation as the room-name anchors below):
...
- **Moved room-name labels** — drag a name off its centroid; the position is stored as **% of the content box** (container-independent, so one key works for panel + card), key `evcc_map_room_names_<vac>_<mapId>`. A tap (no drag) still forwards to the polygon's single-click select; dropping a name back near its centroid clears the anchor (auto-placement). Both kinds of name label are draggable (`.evcc-map-label--draggable`, `pointer-events:auto`) — the segment labels and the device-room fallback labels alike (`_renderDeviceRoomLabels` emits the same class and `data-action="room-name-drag"`); a device-room label carries no `data-segment`, so its tap selects by room *number* (`_selectDeviceRoomFromLabelTap`) rather than by polygon segment.
