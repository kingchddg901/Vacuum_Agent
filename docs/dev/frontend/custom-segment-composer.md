...
Two per-vacuum display toggles live in the Rooms-view map toolbar (`_renderRoomsViewToggle` in `src/renderers/rooms.js`), independent of the composer. The companion controls' markup (select + size slider + paw toggle + follow) is emitted by `_renderMapAnimalControls` in `src/renderers/map.js`, extracted so BOTH the panel toolbar and the embedded dashboard map host (`src/cards/vacuum-map-host.js`) render the identical `_bindMap`-wired controls; the texture toggles' markup stays in `rooms.js`:
