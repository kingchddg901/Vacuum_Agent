...
when a rule fires mid-job. Covered by **61 tests across 3 files**.
...
|---------------|------:|----:|------------|-------|
| `run_plan.py` | 584 | 93% | `test_run_plan_start_plan.py`, `test_run_plan_manager.py`, `test_run_plan_helpers.py` (unit) | int + unit |
