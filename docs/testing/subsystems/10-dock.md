...
timestamps + debounced counters. Covered by **24 tests in 1 file**.
...
|---------------|------:|----:|-----------|-------|
| `manager.py` | 164 | 92% | `tests/integration/test_dock_manager.py` | integration |
...
- **Entity resolution** — `_get_dock_action_entity` resolves a present button by
  candidate id.
