...
| UAC-20 | **a borrowed counter's reset does not zero the borrower** — the failure that deleted `proxy_for` on 2026-09-12, and the proof it cannot recur. Eufy publishes no swivel-wheel counter, so the wheel borrows the filter's; under the old point-value maths resetting the filter dropped 46 → 0, the clamp absorbed it, and the wheel read BRAND NEW. The borrow was never the bug — a proxied part's reset has exactly the shape `observe` already refuses to count. Asserts the total *through* the reset, not merely after it: a version that zeroes then counts forward still passes a final-value check. |
