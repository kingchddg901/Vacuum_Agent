...
- **Zone cleaning.** Draw a box on the map to clean just an area, **save** it as a named zone to re-clean by name, or drop a saved zone into a run as a sequenced, learnable **step** — so one job cleans rooms *and* a zone in order. See [Zones](04a-zones.md).
