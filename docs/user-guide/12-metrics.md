...
| Health % | CV-regime charge speed (80→90 % phase, the resistance proxy) compared to *your install's* baseline. Headline alias of the `_cv_charge_speed` sensor. 100% means matching the baseline; below 100% means slower-than-baseline. The companion `_cc_charge_speed` sensor tracks the CC regime (capacity proxy) for the full picture. Until a figure is available, the chip explains why: a brand-new install with no charge history shows "Building baseline"; an install with history but no qualifying session yet (none has started ≤ 50 % and reached ≥ 90 %) shows a plain-language reason instead. See [13-battery-health.md](13-battery-health.md#seeding-the-baseline-faster) for how to force the seeding instead of waiting, and the `eufy_vacuum.battery_rebaseline` service for use after a battery swap. |
