...
`_battery_health` is clamped at 100 % for the headline, but it exposes the raw (possibly over 100) CV charge-speed value on its `uncapped_pct` attribute for anyone who wants the unclamped signal. It also carries `baseline_cv_min_per_pct`, `baseline_cc_min_per_pct`, `baseline_session_count`, `baseline_anchored_at`, and `completed_sessions`. When no health figure is available *and* there's enough charge history to say why, the paired `health_unavailable_reason` / `health_unavailable_reason_text` attributes carry a stable code plus a plain-English explanation (for example, no completed charge has both started low enough and ended high enough to qualify, or a qualifying charge's computed figure was rejected as implausible). A brand-new install with no completed charges yet, or one right after `battery_rebaseline`, leaves both attributes null instead.
