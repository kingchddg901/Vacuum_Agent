...
- Plural agreement to verify: `learning.incomplete_title` (few form), `rooms.n_blocked`/`n_included`. (~~`mapping_review.badge_n_excluded` — elided-noun gender, pick исключён/исключена by referent~~ — retired, see note above.)

## de (German)
- ~~`mapping_review.baseline` — "Grundlinie" reads as a chart baseline; sense is the protected anchor/oldest run. → "Referenzlauf" / "Basislauf".~~ (retired — see note above.)
...
- Add `{one}` forms for count=1 agreement (only `{other}` shipped): `maintenance.due_in_weeks`/`due_in_months`, `external_jobs.segments_merged`, `base_station.recorded_count`, `rooms.n_blocked`/`n_included`. (~~`mapping_review.badge_n_excluded` — also verify masc/fem referent~~ — retired, see note above.)
