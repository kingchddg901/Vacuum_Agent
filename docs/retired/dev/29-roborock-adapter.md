...
| `const.py` | Identity strings (`DOMAIN`, `NAME`, `SUPPORTED_TESTED_MODEL`), the brand-level `ADAPTER_ID`, `LOW_BATTERY_THRESHOLD_PERCENT`, and **`UPSTREAM_PLATFORMS`** — the HA integration domain(s) providing this brand's vacuum entity (`("roborock",)`). That tuple IS the brand's identity claim; `brands.py` matches it against the entity registry's `platform`. There is no `is_roborock_vacuum` detector — see [21-adapter-system §6.1](21-adapter-system.md). |
...
| `model_catalog.py` | `profile_for_model()` — maps `roborock.vacuum.s6` → the s6 **capability profile** (a dict: `family`/`display_name`/`has_mop`/`mop_settable`/`has_path_control`), not a family string like Eufy's `detect_model_family()`. ⚠ Its `has_dock` key is a **fallback only** and is `False` on every profile by design — the dock is a separate device that the same robot model ships with in several tiers, so a per-model table cannot answer it. `supports_segments` is likewise **inert**: core derives it (`supports_segments = supports_rooms`) from live entities and never reads the catalog's copy. |
| `dock.py` | Dock identification. Finds the dock device by identifier (`{duid}_dock`, a second device in the same config entry), reads its `model_id` (the numeric `dock_type`, or the literal `"Unknown"`), and asks the vendor's own `RoborockDockFeatures` what that dock can do. Returns `None` for **undetermined** — never a guess — so the caller keeps its conservative default. |
| `maintenance_components.py` | **14** entries: 4 robot life-tracked (main/side brush, filter, sensor — device-owned `*_time_left` countdowns with reset buttons) + **2 dock life-tracked** (`cleaning_brush`, `strainer`, on the DOCK device, reachable only via the config-entry scope of the sibling sweep) + 8 `maintenance_only` guide-only cleanables (dustbin, mop_cloth, water_filter, caster_wheel, main_wheel; dock/station ones family-gated). The two dock entries are **self-gating** — they resolve to `None` on a dockless unit and produce no row and no button, so they are inert on the S6. |
