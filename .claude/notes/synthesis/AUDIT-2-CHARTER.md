...
## 2c. Findings surfaced DURING the audit are tracked, not carried (Chris, 2026-08-05)

**The rule:** any finding discovered while doing audit work — by writing a fix, by running the
suite, by deploying, by verifying something else — is filed to the ledger **at the moment it is
found**, with the same fields as a finding the audit set out to look for. It is never left in
the conversation, never left "for the write-up", and never left as a line in a closing report.

**Why this is a gate and not a nicety — 2026-08-05 produced four in one session:**

| finding | what surfaced it |
|---|---|
| `PHASE-ATTR-2` | the `PHASE-ATTR-1` fix — self-inflicted, caught by a test written for the harvester |
| `SETTING-SCALE-1` | Chris rejecting a proposal to DELETE `by_clean_times` as an orphan |
| `BATT-CV-1` | RF-36's hardware verification, the first time those fields ever held a value |
| `BATT-ZONE-1` | deploying the `BATT-CV-1` guard and reading the sensors afterwards |

Three of those four were found by *acting on* the audit rather than by auditing, which is the
point: fixing, deploying and verifying are the highest-yield discovery surfaces in the whole
campaign, and they are the ones with no ledger discipline attached to them.

**The one that nearly proves the rule by breaking it:** `_pending_fast_rollover` is read in
`jobs/active_job.py` and written NOWHERE — an entire rollover branch is dead. It was found while
chasing `ROOM-FLICKER-1`, was not the thing being chased, and survived only because it reached a
closing summary. Had that session ended differently it would have evaporated. **File it when you
see it.**

**Provenance is a required field, not a courtesy.** Record WHAT surfaced it — the fix, the
deploy, the verification — because that is the signal the next campaign scopes from. "Found by
deploying a guard and reading the sensors" tells you to deploy-and-read on purpose; "found in
`battery/manager.py`" tells you nothing you did not already know.

**File to the INPUT, never the render.** New findings go in `_direct_reads.json`; adjudications
go in `_adjudicated_findings.json`. `OPEN-FIX-CHECKLIST.md` is generated by `_gen_checklist.py`
and a hand-edit there is invisible to the generator — proven the same day, when regenerating the
file revealed **26 ticked entries with no JSON backing**, real hand-verifications that a
regeneration would have discarded silently (inventoried in `_unbacked_ticks.json`).

**Check for drift with the tool, not by eye:** `python .claude/notes/_verify_ledger.py` flags
findings the ledger calls open whose id appears in shipped code. It is triage, not proof — a
missing marker proves nothing, as the 2026-08-04 pass learned by putting two already-fixed
findings back on the board using exactly that reasoning.
