...
1. **Net-shrink per closure.** Every closed round leaves the section strictly smaller than the **live file at apply time** — the reconciled base of rule 13, not the snapshot the trim was taken from. This kills reviewer invariant-inflation structurally: a document that must shrink to close cannot be regrown for points. Corrective or clarifying additions (see the removal manifest under Trim Agent) count against the same budget — they must fit inside the shrink, which keeps invariant-inflation and addition-padding equally unprofitable.

    The baseline is the LIVE file rather than the snapshot because the two readings are jointly unsatisfiable whenever drift exceeds the shrink. Rule 13 requires live hunks that landed during the loop to be re-applied onto the candidate; measured against the snapshot, a drift larger than the trim's shrink would then force the coordinator either to break this rule or to discard shipped, proven work to fit — which is the exact failure rule 13 exists to prevent. Measuring against the live file makes both rules satisfiable at once and keeps the shrink honest, since the file it is compared to is the one that actually exists. CAL-23 escaped this only by margin: base 475, live 488 (its own first-blood correction), candidate 415 — reconciled 428, still under 475. A larger drift inverts it.
...
3. **Fan-out order.** The doc numbers are landing order; the module graph is mutually recursive (5 of 26 dependency-free, the rest one cluster — see `DOC-DEPENDENCY-MAP.md`), so no topological ablation order exists. Proof composition does not need one: closures lean on other docs' interface statements, which all exist from day one. The fleet runs atom-first, then the README's reading order (doc 32): the atom's sections — 21/22 (adapter), 07 (queue/dispatch), 08 (rooms), 06 (active_job) — close first, because with just those proven a rebuilt system CLEANS; then the remainder top-to-bottom (01 → 02 → 04 → 05 → 30 → 09…15/31 → 16-18 → 25/26/29 → 28).

   **AGGREGATOR DOCS ABLATE LAST — after every subsystem that contributes to them.** Doc 03 (data model) is the worked example and is deliberately NOT in the atom list above despite being first in the recovery progression; importing it from there was the three-sequences conflation this section warns against, one paragraph down. The persisted store is an ACCRETION, the same property already established for the manager hub: `core/storage.py` seeds a union of per-subsystem buckets (`theme`, `maintenance`, `onboarding`, `error_tracker`, `dock_events`, plus `battery`/`adapters` created lazily by their owners on first write), and 03 §1 delegates each one explicitly — "see 12-battery-system.md", "domain bucket; see dock/". Its own §7 row calls it the aggregator tier and the LAST backend doc, closed as an INDEX; the backend audit sequenced it there for this reason.

   Ablating an aggregator early is doubly wrong. The trim agent would minimize statements whose authority lives in a doc not yet ablated, and every later subsystem closure that alters its bucket's shape changes the aggregator's PROVIDES surface and flags it provisional — with ~8 contributing buckets, up to 8 re-runs of the most expensive kind. Ablate the owners, then the index, once.

   The general rule: a doc whose sections are OWNED ELSEWHERE is ablated after its owners. Identify one by delegation density — a section that points at other docs for its shapes rather than stating them is an index, not a specification. The compensating rule that replaces sequencing: **any closure that alters a doc's PROVIDES surface flags every dependent doc's closure provisional** (dependents per `DOC-DEPENDENCY-MAP.md`). Trims must preserve interface statements regardless — they are contract.
...
- **Fable is used at exactly one point, by escalation rather than by staffing:** contested forensic rulings — void/no-void decisions with payout consequences that survive the Opus forensic audit.

  Fable spend is measured in rulings, not rounds.

  The second reserved point — a Fable review pass over the doc-23 calibration outcome before fan-out — is **DISCHARGED and struck, not pending** (2026-08-07). Testing whether that tier was needed at all was one of the two questions the calibration existed to answer, and Fable coordinated CAL-23 end to end; the answer it returned was NO. Everything after round-1 adjudication was templatable coordination, so the fleet is an Opus coordinator + the scripted economy + Sonnet loops. Struck rather than annotated, per rule 14: left standing as an unmet prerequisite it would block every future fan-out run whose reader loads this section without the ledger.
