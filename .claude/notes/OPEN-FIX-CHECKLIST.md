...
- [x] **Pose sampler predicates** -- **REJECTED 2026-08-04 (Chris agreed).** Not a deferral any more: the non-unification is CORRECT. `_DISPATCHED_IN_FLIGHT_STATUSES` counts `paused` as in flight, and a paused robot is stationary — sampling it injects a run of identical points. That lands differently in the two consumers: the swept-area ATTRIBUTION would accumulate dwell at a fixed point (bad — it decides which room got cleaned), while the live TRAIL arguably wants the dot to persist. "Is a job in flight" and "should we sample pose" are different questions that merely overlap; unifying them would centralize the vocabulary, not the question. Do not re-point these.
- [ ] **Roborock room migration** -- **RE-SCOPED 2026-08-04 (Chris agreed): DO NOT MIGRATE.** Room creation now resolves defaults from the brand catalog (`resolve_new_room_defaults_for_vacuum`); rooms created before that took framework defaults. VERIFIED why a migration cannot be safe: both catalogs use the IDENTICAL profile names (`vacuum_quick`, `vacuum_deep`, `vacuum_mop_quick`, `vacuum_mop_deep`) and differ only in the values inside, so a mis-branded room is undetectable by name — and nothing distinguishes "a stale default from the wrong brand" from "a value the user deliberately set that happens to equal it". Same distinguishability wall as A4-SETUP-6. The honest shape is SURFACING: flag rooms whose stored values are not in the brand's vocabulary and let the user re-apply a profile. Never rewrite them.
- [x] **Three card strings untranslated** -- `common.service_failed`, `learning.room_skipped`, `learning.run_incomplete_toast` are English-only across the 17 non-English locales.
- [x] **Card: the two failure-renders-as-success paths (FE-ERR-1 / MZ-2)** -- CLOSED 2026-08-04, and the "blocked on a backend `supports_response` change" note was stale: job_control registers supports_response=True on every start_*, FE-ERR-1's card half landed with CARD-1, and MZ-2 was always reachable through the generic wrapper because start_zone_clean refuses with `{success:false}` ([MZT-1..4] pin it). The REAL forgotten sibling was `start_run_profile`, which refuses with `{started:false}` — a shape only startCleaning inspected, so a refused run-profile start was silent. callService now asks once for both shapes ([CRS-6..8]).
- [ ] **Card: the qualification gap** -- 4 of 6 CLOSED 2026-08-04: REV-6 (absent battery), REV-5 (silent truncation), CC-5 (clean-mode chip), CENSUS-6 (server-baked English). **REMAINING: CENSUS-5** — the whole error-tracker read surface (a binary_sensor + two service responses) is unconsumed by the card; a feature to build, not a bug to fix, and the review error badge may already cover the useful part. **REV-2** — the confirm wizard pre-fills every room from a GUESS and submits on two clicks; a design question about how far to trust an inference. **REV-6, REV-5 and CC-5 CLOSED 2026-08-04** — absent battery no longer renders as "Battery 0"; the run list states its own truncation; and the matcher's clean-mode chip compares canonically (it compared the DISPLAY form the store writes against the adapter's raw token, so the chip just clicked rendered inactive). **CENSUS-6 CLOSED 2026-08-04** — attention_summary is composed card-side from attention_count (real plurals, not "item(s)"), and the three `*_status_label` sites now resolve the CODE first with the server label as fallback; they preferred the English label over a translation the card already had. Still open in this cluster: **CENSUS-5** (the whole error-tracker read surface — a binary_sensor and two service responses — is unconsumed by the card; a feature to build, not a bug to fix) and ~~REV-2~~ **CLOSED 2026-08-04** (Chris: show the guess, don't pre-select). shortlist[0] is now SUGGESTED and Confirm is gated until every room is actively chosen — these runs feed LEARNING, so a wrong guess accepted by reflex became a wrong timing sample on the wrong room.
- [x] **Card: surface captured run errors (`run_errors`)** -- The backend now carries app-started-run error evidence end to end. Nothing displays it.
- [x] **OpenDyslexic font support** -- **SHIPPED 2026-08-04** (dfff15f P1+P2, 818ad37 P3, dd2ec11 P4). Served not embedded — the real faces are 115+120KB, ~314KB base64'd against the design's ~100KB estimate, and the cards bundle loads on EVERY HA page. OFL.txt shipped beside the woff2 (the release zip has none). REMAINING: the gallery baseline repin (rooms-opendyslexic) is a deliberate hand-off — a new visual baseline is a claim about what correct looks like. Contract settled — English-only gate, one token override, glyph coverage proven per locale before offering another. No code written.
