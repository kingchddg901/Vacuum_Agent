...
- [ ] **Card: the qualification gap (CC-5)** -- Surface provenance, truncation and absent data honestly rather than as confident values. **REV-6 CLOSED 2026-08-04** (absent battery no longer renders as "Battery 0"). Still open in this cluster: CENSUS-5 (error-tracker read surface unconsumed), CENSUS-6 (server-baked English labels rendered), REV-2 (confirm wizard pre-fills from a guess), REV-5 (run list silently truncated to 50 while the headline shows the untruncated count), CC-5 proper (clean-mode chip compares a display-canonicalised value against the raw field).
