...
- [ ] **Card: the qualification gap (CC-5)** -- Surface provenance, truncation and absent data honestly rather than as confident values. **REV-6, REV-5 and CC-5 CLOSED 2026-08-04** — absent battery no longer renders as "Battery 0"; the run list states its own truncation; and the matcher's clean-mode chip compares canonically (it compared the DISPLAY form the store writes against the adapter's raw token, so the chip just clicked rendered inactive). Still open in this cluster: **CENSUS-5** (the whole error-tracker read surface — a binary_sensor and two service responses — is unconsumed) and **CENSUS-6** (server-baked English `attention_summary` / `*_status_label` rendered verbatim in 18 locales); **REV-2** (the confirm wizard pre-fills every room from a GUESS and submits on two clicks).
