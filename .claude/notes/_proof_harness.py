...
class Proof:
    ...
    def __init__(self, packet: str, title: str) -> None:
        ...
        self._stubs: list[tuple[str, Any]] = []       # (target, mock) via self.patch()
        self._recusals: list[str] = []                # contract mismatches etc.
        print(f"=== {packet} — {title} ===")

    # -- v2: harness-derived admissibility (audit-2 charter §4 delta 6) --------

    def patch(self, target: str, *args: Any, **kwargs: Any) -> Any:
        """The ONE sanctioned stub path. Registers the stub for invocation
        tracking: a stub that production never calls means the proof watches a
        symbol production no longer uses (the moved-call-site staleness class),
        and finish() quarantines the whole file. Derived, never declared — using
        this instead of unittest.mock directly is what buys the check."""
        patcher = _mock.patch(target, *args, **kwargs)
        m = patcher.start()
        self._stubs.append((target, m))
        return m

    def require_contract(self, name: str, written_against: int) -> None:
        """Recuse this proof if production's mechanism contract has moved on.

        For MECHANISM-sensitive proofs only (the retired-mechanism class — design
        changed without structure changing, so nothing textual fails). Production
        holds the constant in custom_components/eufy_vacuum/contract_versions.py;
        bumping it there makes every proof written against the old mechanism
        recuse itself loudly instead of certifying the wrong thing.
        """
        try:
            from custom_components.eufy_vacuum.contract_versions import CONTRACT_VERSIONS
        except ImportError:
            self._recusals.append(f"contract registry missing (wanted {name} v{written_against})")
            return
        actual = CONTRACT_VERSIONS.get(name)
        if actual is None:
            self._recusals.append(f"contract '{name}' not in registry (proof written against v{written_against})")
        elif actual != written_against:
            self._recusals.append(
                f"contract '{name}' is v{actual}, proof written against v{written_against} — mechanism moved on"
            )
    ...
    def finish(self) -> int:
        ...
        uninvoked = [t for t, m in self._stubs if not getattr(m, "called", False)]

        # Quarantine (v2): any UNEXPECTED shape, uninvoked stub, or contract
        # recusal makes the WHOLE file inadmissible — render 0 of N admissible,
        # never a bankable mixed tally, so no case in a compromised file counts
        # as evidence. BEFORE never quarantines: a healthy partially-landed
        # packet legitimately reports mixed BEFORE · AFTER.
        if counts["UNEXPECTED"] or uninvoked or self._recusals:
            for name, v in self._verdicts:
                if v == "UNEXPECTED":
                    print(f"    unexpected: {name}")
            for t in uninvoked:
                print(f"    UNINVOKED STUB: {t} — production never called it; "
                      f"the proof watches a symbol production no longer uses")
            for r in self._recusals:
                print(f"    RECUSED: {r}")
            print(f"\n=== {self.packet}: QUARANTINED — 0 of {len(self._verdicts)} admissible ===")
            return 1
        print(f"\n=== {self.packet}: "
