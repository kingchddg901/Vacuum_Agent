...
PACKET GAP (report to the author): the packet's expected_before lists
"hold accrued 300s" as its third fragment, but no finding in its own
finding_ids list matches that description — DQ-PH-6 is the native-room carry-
over above. Case 3 proves DQ-PH-6. Whichever finding "hold accrued 300s" refers
to is NOT covered by this proof and needs naming before RP-012 is assigned.
...
async def main() -> int:
    ...
    print("\nNOTE: the packet's third expected_before fragment "
          "\"hold accrued 300s\" matches no finding in RP-012's finding_ids; "
          "case 3 proves DQ-PH-6 instead. Flag to the packet author.")
    return proof.finish()
