...
def case_progress_tick_skips_external(proof: H.Proof) -> None:
    ...
    # STALE-PROOF REPAIR, 2026-08-03. This watched `get_job_progress_snapshot`,
    # which WAS the listener's call when the proof was written. RP-037 b/3
    # (7b53ed4, SNAP-2) then made that method a pure read and moved the ticker
    # onto `apply_job_progress_tick` — the explicit call that actually advances
    # state. The proof kept watching the old name, so a CORRECT repair reported
    # snapshots=[] and UNEXPECTED SHAPE.
    #
    # Both are stubbed now: the tick is what production calls, and the snapshot
    # stays watched so this reports honestly if the entry point moves again.
    # The assertion still measures the packet's requirement — "the tick covered
    # the external run" — not an implementation detail of which method does it.
    mgr.apply_job_progress_tick = lambda **kw: snapshots.append(
