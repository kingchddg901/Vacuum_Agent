...
class MappingManager:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        # WHY separate: boundary traces derive room polygons from perimeter walks;
    ...
    def start_trace_capture(
        ...
        *,
        ...
        """Start a raw trace capture session.

        room_id is optional — the room may not be known at capture time.
        Phase 2 handles room association. If a session is already active
        for this (vacuum, map) pair it is discarded and the caller is
        informed via previous_cancelled in the return payload.
        """
        return self._trace_capture.start(
            vacuum_entity_id=vacuum_entity_id,
            map_id=str(map_id),
            room_id=room_id,
        )

    def append_trace_sample(
        self,
        *,
        vacuum_entity_id: str,
        map_id: str,
        ...
        """Append one raw position sample to the active capture session.

        Silent no-op if no session is active. Returns True if appended.
        Called by the tracker once wired in a later phase. Also callable
        directly for testing without tracker wiring.
        """
        return self._trace_capture.append_sample(
            vacuum_entity_id=vacuum_entity_id,
            map_id=str(map_id),
            x=x,
            y=y,
    ...
    def stop_trace_capture(
        ...
        *,
        ...
        """Stop the active capture session and persist the TraceRun.

        Returns a summary with run_id, sample_count, and path.
        Returns stopped=False if no session was active.
        """
        return self._trace_capture.stop(
            vacuum_entity_id=vacuum_entity_id,
            map_id=str(map_id),
        )

    def cancel_trace_capture(
        self,
        *,
        vacuum_entity_id: str,
        map_id: str,
    ) -> dict[str, Any]:
        """Discard the active capture session without writing.

        Returns cancelled=False if no session was active.
        """
        return self._trace_capture.cancel(
            vacuum_entity_id=vacuum_entity_id,
            map_id=str(map_id),
        )

    def get_trace_capture_status(
        self,
        *,
        vacuum_entity_id: str,
        map_id: str,
    ) -> dict[str, Any]:
        """Return current capture session status for this (vacuum, map)."""
        summary = self._trace_capture.active_session_summary(
            vacuum_entity_id=vacuum_entity_id,
            map_id=str(map_id),
        )
        return {
    ...
    def list_trace_runs(
        ...
        *,
        ...
        """Return all stored run_ids for one vacuum, sorted ascending.

        Does not load sample data — returns IDs only for efficient listing.
        """
        slug = _vacuum_slug(vacuum_entity_id)
        run_ids = list_trace_run_ids(self._base_dir, slug)
        return {
            "vacuum_entity_id": vacuum_entity_id,
    ...
    def get_trace_run(
        ...
        *,
        ...
        """Load and return one stored TraceRun by run_id.

        Returns found=False if the run does not exist.
        """
        slug = _vacuum_slug(vacuum_entity_id)
        run = load_trace_run(self._base_dir, slug, run_id)
        if run is None:
            return {
                "found": False,
                "vacuum_entity_id": vacuum_entity_id,
    ...
    def delete_trace_run_by_id(
        self,
        *,
        vacuum_entity_id: str,
        run_id: str,
    ) -> dict[str, Any]:
        """Delete one stored TraceRun by run_id.

        Returns deleted=False if the run does not exist.
        """
        slug = _vacuum_slug(vacuum_entity_id)
        deleted = delete_trace_run(self._base_dir, slug, run_id)
        return {
            "deleted": deleted,
            "vacuum_entity_id": vacuum_entity_id,
            "run_id": run_id,
    ...
    # ------------------------------------------------------------------
    # Trace-based transform pipeline — Phase 2: Coarse run review
    # ------------------------------------------------------------------

    def review_trace_run_for_room(
        self,
        *,
        vacuum_entity_id: str,
        map_id: str,
        ...
        Room linkage is assumed correct. Measures trace quality only — does not
        re-evaluate room assignment. Returns a RunReviewResult dict with verdict
        (accepted / needs_refine / rejected / error) and diagnostics.
        ...
        # Load the vacuum-space polygon from boundary-trace storage.
        # review_trace_run returns an error verdict if the polygon is absent.
        map_data = self._load_map_data(vacuum_entity_id, str(map_id))
    ...
    def segment_trace_run_for_room(
        self,
        *,
        vacuum_entity_id: str,
        map_id: str,
        ...
        """Load a TraceRun and segment it into candidate behavioural units.

        When room_id is supplied and a boundary exists, the vacuum-space polygon
        is passed to enable the BOUNDARY_CROSSING signal. Without a polygon,
        only PAUSE and speed/density signals are used. Returns a SegmentationResult.
        Does not persist — this is a read-only analysis pass.
        """
        slug = _vacuum_slug(vacuum_entity_id)
        run = load_trace_run(self._base_dir, slug, run_id)
        if run is None:
        ...
        # Load vacuum-space polygon to enable the BOUNDARY_CROSSING signal.
        polygon_vacuum: list[list[float]] | None = None
        if room_id is not None:
            map_data = self._load_map_data(vacuum_entity_id, str(map_id))
            rooms = map_data.get("rooms", {})
            room_entry = rooms.get(str(room_id), {})
            boundary = room_entry.get("boundary", [])
            if isinstance(boundary, list) and len(boundary) >= 3:
                polygon_vacuum = boundary

        return segment_trace_run(run, polygon_vacuum)
