...
def _build_release_theme_colors(
    *,
    ...
    return {
        ...
        "--evcc-learning-confidence-low-bg": "color-mix(in srgb, var(--evcc-sem-error) 14%, transparent)",
        ...
        "--evcc-learning-confidence-neutral-bg": "color-mix(in srgb, var(--evcc-surface-input) 96%, transparent)",
...
def _ensure_preloaded_theme_library(theme_data: dict[str, Any]) -> None:
...
class EufyVacuumManager:
    ...
    async def async_initialize(self) -> None:
        ...
        self.data["theme"].setdefault("default_theme_id", None)
        self.data["theme"].setdefault("vacuums", {})
        _ensure_preloaded_theme_library(self.data["theme"])
    ...
    def register_theme_update_callback(self, callback) -> None:
        ...
        if callback not in self._theme_update_callbacks:
            self._theme_update_callbacks.append(callback)

    def unregister_theme_update_callback(self, callback) -> None:
        """Unregister a theme update callback."""
        if callback in self._theme_update_callbacks:
            self._theme_update_callbacks.remove(callback)

    def _notify_theme_updated(
        self,
        *,
        vacuum_entity_id: str | None = None,
    ) -> None:
        """Fire all registered theme update callbacks."""
        for cb in list(self._theme_update_callbacks):
            try:
    ...
    def get_job_progress_snapshot(
        ...
        *,
        ...
        # the robot has been in the room for >= 2× the timing threshold.
    ...
    def _get_theme_data(self) -> dict:
        ...
        Preloaded themes are seeded once in ``async_initialize()`` and not
        repeated here.  The setdefault calls below are cheap structural guards.
        """
        self.data.setdefault("theme", {})
        self.data["theme"].setdefault("library", {})
        self.data["theme"].setdefault("default_theme_id", None)
    ...
    def _generate_run_profile_id(self) -> str:
        """Generate a stable, sortable unique saved-run profile ID."""
        from datetime import datetime as _dt
        return f"run_{_dt.now().strftime('%Y%m%dT%H%M%S%f')}"

    def _generate_room_profile_id(self) -> str:
        """Generate a stable, sortable unique custom room-profile ID."""
        from datetime import datetime as _dt
        return f"room_profile_{_dt.now().strftime('%Y%m%dT%H%M%S%f')}"

    def _generate_room_rule_id(self) -> str:
        """Generate a stable, sortable unique room-rule ID."""
        from datetime import datetime as _dt
        return f"rule_{_dt.now().strftime('%Y%m%dT%H%M%S%f')}"
    ...
    def _load_room_history_cache_sync(self, vacuum_entity_id: str) -> dict[str, Any]:
        """Build and return room-history data from disk without touching self.data.

        Runs in an executor thread — all state is local until the caller writes
        the result back to self.data on the event loop.
        """
        ...
        try:
            ...
            _LOGGER.exception(
                "Failed rebuilding room history cache for %s",
                vacuum_key,
            )
    ...
    def _ensure_room_history_cache(
        self,
        *,
        vacuum_entity_id: str,
    ) -> None:
        """Ensure room-history cache preload has been scheduled for one vacuum."""
        vacuum_key = str(vacuum_entity_id)
        if vacuum_key in self._room_history_cache_ready:
            return
        if vacuum_key in self._room_history_cache_loading:
    ...
    def _ingest_jobs_index_entry_into_room_history(
        ...
        *,
        ...
        """Merge one derived jobs-index entry into the room-history cache."""
        ...
        history_root = _history_root if _history_root is not None else self.data.setdefault("room_history", {})
        ...
        for room in rooms:
            ...
            def _is_newer(field_name: str) -> bool:
            ...
            last_cleaned_at = room.get("last_cleaned_at")
            if last_cleaned_at and _is_newer("last_cleaned_at"):
                next_entry["last_cleaned_at"] = last_cleaned_at
                next_entry["last_job_mode"] = room.get("clean_mode")

            last_vacuumed_at = room.get("last_vacuumed_at")
            if last_vacuumed_at and _is_newer("last_vacuumed_at"):
                next_entry["last_vacuumed_at"] = last_vacuumed_at

            last_mopped_at = room.get("last_mopped_at")
            if last_mopped_at and _is_newer("last_mopped_at"):
                next_entry["last_mopped_at"] = last_mopped_at
    ...
    def _room_mode_uses_vacuum(clean_mode: Any) -> bool:
        """Return whether one room mode includes vacuuming."""
    ...
    def _room_mode_uses_mop(clean_mode: Any) -> bool:
        """Return whether one room mode includes mopping."""
        mode = str(clean_mode or "").strip().lower()
    ...
    def _ingest_completed_job_into_room_history(
        ...
        *,
        ...
        for room in resolved_rooms:
            ...
            if _is_newer("last_cleaned_at"):
                next_entry["last_cleaned_at"] = ended_at
                next_entry["last_job_mode"] = clean_mode
            if clean_mode and self._room_mode_uses_vacuum(clean_mode) and _is_newer(
                "last_vacuumed_at"
            ):
                next_entry["last_vacuumed_at"] = ended_at
            if clean_mode and self._room_mode_uses_mop(clean_mode) and _is_newer(
                "last_mopped_at"
            ):
    ...
    def _update_room_rule_status_snapshot(
        ...
        *,
        ...
        """Store the latest per-room rule/preflight evaluation snapshot."""
        ...
        for room_key, room in managed_rooms.items():
            ...
            next_entry = {
                ...
                "last_warning_codes": list(preflight.get("warnings", []))
    ...
    def get_room_rule_status(
        ...
        *,
        ...
        return {
            ...
            "last_triggered_rule_ids": list(entry.get("last_triggered_rule_ids", []))
            if isinstance(entry.get("last_triggered_rule_ids"), list)
            else [],
            "last_modifier_changes": dict(entry.get("last_modifier_changes", {}))
            ...
            "last_warning_codes": list(entry.get("last_warning_codes", []))
    ...
    def get_runtime_path_block_report(
        ...
        *,
        ...
        validation = self._validate_room_access_graph(managed_rooms=managed_rooms)
        if self._structural_access_graph_issues(validation):
            return None
        ...
        grants_map, requires_map = self._build_room_access_views(
            managed_rooms=managed_rooms
        )
        ...
        for room_id in queue_room_ids:
            ...
            for rule in room.get("rules", []):
                ...
                entity_id = str(rule.get("entity_id", "")).strip()
                if not entity_id or not self._room_rule_matches(rule):
                    continue
    ...
    def _get_theme_library_entries(self) -> dict[str, dict[str, Any]]:
        ...
        normalization (save / import / delete) keeps stored entries clean
        without touching storage on every read.
        """
        theme = self._get_theme_data()
    ...
    def save_theme_as_new(
        ...
        *,
        ...
        self._notify_theme_updated(vacuum_entity_id=vacuum_entity_id)
    ...
    def rename_theme(
        ...
        *,
        ...
        self._notify_theme_updated(vacuum_entity_id=None)
    ...
    def delete_theme(self, *, theme_id: str) -> dict[str, Any]:
        ...
        for vac_id, vac_data in theme["vacuums"].items():
