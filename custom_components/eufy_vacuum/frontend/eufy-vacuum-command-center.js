...
config/eufy_vacuum/battery/${this.escapeHtml(S)}/samples.jsonl</pre>
    ...
    `:""},i.renderRoomsActionBar=function(e,t,r,a,n){let c=r===1?"1 room":`${r} rooms`,s=(Array.isArray(a)?a:[]).filter(T=>T.enabled),o=e?n?"evcc-chip--start-warn":"evcc-chip--start":"disabled",l=this.card?._state,d=!!l?.hasActiveRun?.(),u=Number(this.card?._learningController?.getJobProgressPercent?.()??0),m=Array.isArray(l?.learningRoomTimeline?.())?l.learningRoomTimeline():[],p=l?.learningCompletedRooms?.()||[],v=new Set(p.map(T=>String(T.room_id))),f=s.reduce((T,U)=>{let Y=String(U.id),J=m.find(Q=>String(Q.room_id)===Y),Me=this.card?._state?.roomEstimateForRoom?.(U.id)??null,ue=Number(J?.minutes??Me?.minutes);return Number.isFinite(ue)?T+ue:T},0),h=Number(l?.dashboardPlannedJobEstimateTotalMinutes?.()),_=Number.isFinite(h)&&h>0?h:f,S=_>0?this._formatLearningDuration(_):null,b=l?.startConfirmation?.()??null,g=l?.startPreflight?.()??b?.preflight??null,R=!!l?.startRequiresConfirmation?.(),y=!!l?.cancelRunRequiresConfirmation?.(),E=!!l?.clearQueueRequiresConfirmation?.(),M=!!l?.hasActiveRun?.(),G=!!l?.canPauseRun?.(),z=!!l?.canResumeRun?.(),he=y?"Confirm Cancel":M?"Cancel Run":R?"Confirm Start":"Start Cleaning",D=y?"evcc-chip--start-warn evcc-chip--confirm-flash":M?"evcc-chip--cancel-run":R?"evcc-chip--start-warn evcc-chip--confirm-flash":o,me=R||y,re=Array.isArray(g?.blocked_rooms)?g.blocked_rooms:[],ne=Array.isArray(g?.modified_rooms)?g.modified_rooms:[],ge=Array.isArray(g?.warnings)?g.warnings:[],be=l?.startMopCarpetWarning?.()??null,_e=l?.startOrderAdvisory?.()??null,L=l?.strictOrder?.()??!1;return`
  ...
  `},i.renderRoomCard=function(e,t){let r=this._normalizeRoomDisplayData(e),a=r.cleanMode?r.cleanModeLabel||this._formatCleanMode(r.cleanMode):null,c=!r.fanSpeed||["off","normal"].includes(String(r.fanSpeed).toLowerCase())?null:r.fanSpeedLabel||this._formatFanSpeed(r.fanSpeed),o=!r.cleanIntensity||String(r.cleanIntensity).toLowerCase()==="standard"?null:r.cleanIntensityLabel||this._formatCleanIntensity(r.cleanIntensity),l=this._isMopMode(r.cleanMode)&&r.waterLevel&&String(r.waterLevel).toLowerCase()!=="off"?r.waterLevelLabel||this._formatWaterLevel(r.waterLevel):null,d=this._isMopMode(r.cleanMode)&&r.edgeMopping?"Edge Mop On":null,u=Number(r.cleanPasses)>1?`${Number(r.cleanPasses)}\xD7 passes`:null,m=this.card?._state?.orderDragItemId?.(),p=this.card?._state?.orderDragOverItemId?.(),v=String(m)===String(r.id)?"evcc-order-drag-source":"",f=String(p)===String(r.id)?"evcc-order-drag-target":"",h=t?.roomEstimateForRoom?.(r.id)??null,_=t?.dashboardPlannedWaterRoomForRoom?.(r.id,r.slug)??null,S="";if(h&&h.error==null){let L=h.source==="learned"?"evcc-room-status--estimate-learned":"evcc-room-status--estimate-default",T=h.source==="learned"?this._formatLearningMinutes(h.minutes):`~${this._formatLearningMinutes(h.minutes)}`,U=[`Estimate: ${this._formatLearningMinutes(h.minutes)}`];h.source&&U.push(`Source: ${String(h.source)}`);let Y=Number(h.battery);Number.isFinite(Y)&&U.push(`Battery: ${Y}`);let J=U.join(" \xB7 ");S=`
    ...
    `}let b="",g="",R="";if(h&&h.error==null&&typeof this.renderConfidenceChip=="function")if(h.source==="learned"){let L=h?.confidence_breakpoint?.ui_variant,T=L==="success"?"Reliable":L==="warning"?"Learning":L==="error"?"Uncertain":null;T&&(b=this.renderConfidenceChip(h.confidence_breakpoint,T,T),L==="success"?g="evcc-room-card--confidence-high":L==="warning"?g="evcc-room-card--confidence-medium":g="evcc-room-card--confidence-low")}else h.source==="default"&&(b=this.renderConfidenceChip({ui_variant:"neutral"},"Unlearned","Unlearned"));let y=String(_?.effective_clean_mode??_?.clean_mode??"").toLowerCase(),E=String(_?.effective_water_level??_?.water_level??"").toLowerCase();if(!!(_?.mop_active||this._isMopMode(y))&&E!=="off"){let L=Number(_.estimated_robot_water_used_ml);Number.isFinite(L)&&(R=`
        <div
  ...
  `},i._normalizeRoomDisplayData=function(e){let t=e?.selected_profile_details??{},r=String(e?.profile_name??e?.profileName??e?.profile??"vacuum_quick"),a=String(e?.clean_mode??e?.cleanMode??t?.clean_mode??"vacuum"),n=String(e?.fan_speed??e?.fanSpeed??t?.fan_speed??""),c=String(e?.water_level??e?.waterLevel??t?.water_level??""),s=String(e?.clean_intensity??e?.cleanIntensity??t?.clean_intensity??""),o=Number(e?.clean_passes??e?.cleanPasses??e?.passes??t?.default_clean_passes??1),l=!!(e?.edge_mopping??e?.edgeMopping??t?.default_edge_mopping??!1),d=String(e?.floor_type??e?.floorType??""),u=String(e?.carpet_type??e?.carpetType??""),m=!!(e?.carpet??(()=>{let v=String(d).toLowerCase();return v==="carpet"||v.startsWith("carpet_")||v.startsWith("carpet-")})()),p=Number(e?.order??e?.displayOrder??e?.position??999999);return{id:e?.id,mapId:e?.mapId??e?.map_id??"",name:e?.name??e?.room_name??"",slug:e?.slug??e?.room_slug??null,enabled:!!e?.enabled,order:Number.isFinite(p)?p:999999,profileName:r,profileLabel:e?.profile_label??e?.profileLabel??e?.selected_profile_label??e?.resolved_profile_label??null,profileSubtitle:e?.profile_subtitle??e?.profileSubtitle??null,lastCleanedAt:e?.lastCleanedAt??e?.last_cleaned_at??null,lastJobMode:e?.lastJobMode??e?.last_job_mode??null,isCustomProfile:r.toLowerCase()==="custom",cleanMode:a,cleanModeLabel:e?.clean_mode_label??e?.cleanModeLabel??t?.clean_mode_label??null,fanSpeed:n,fanSpeedLabel:e?.fan_speed_label??e?.fanSpeedLabel??t?.fan_speed_label??null,waterLevel:c,waterLevelLabel:e?.water_level_label??e?.waterLevelLabel??t?.water_level_label??null,cleanIntensity:s,cleanIntensityLabel:e?.clean_intensity_label??e?.cleanIntensityLabel??t?.clean_intensity_label??t?.path_type_label??null,cleanPasses:Number.isFinite(o)?o:1,cleanPassesLabel:e?.clean_passes_label??e?.cleanPassesLabel??t?.clean_passes_label??null,edgeMopping:l,edgeMoppingLabel:e?.edge_mopping_label??e?.edgeMoppingLabel??t?.edge_mopping_label??null,floorType:d,floorTypeLabel:e?.floor_type_label??e?.floorTypeLabel??null,carpetType:u,carpetTypeLabel:e?.carpet_type_label??e?.carpetTypeLabel??null,carpet:m,selectedProfileDetails:t}},i._isMopMode=function(e){let t=String(e??"").toLowerCase();return t==="mop"||t==="vacuum_mop"||t.includes("mop")||t.includes("wash")},i._roomProfileLabel=function(e){let t=String(e??"").trim();return t?t.toLowerCase()==="custom"?"Custom":t==="vacuum_quick"?"Vacuum Only Quick":t==="vacuum_deep"?"Vacuum Only Deep":t==="vacuum_mop_quick"?"Quick":t==="vacuum_mop_deep"?"Deep":t==="user_1"?"User Profile 1":t.replace(/[_-]+/g," ").replace(/\b\w/g,r=>r.toUpperCase()):"Standard"},i._formatCleanMode=function(e){let t=String(e??"").trim().toLowerCase();return t==="vacuum_mop"||t==="vacuum and mop"?"Vacuum + Mop":t==="vacuum"?"Vacuum":t==="mop"?"Mop":this._formatSettingValue(e)},i._formatFanSpeed=function(e){return this._formatSettingValue(e)},i._formatWaterLevel=function(e){return this._formatSettingValue(e)},i._formatCleanIntensity=function(e){return this._formatSettingValue(e)},i._formatFloorType=function(e){return this._formatSettingValue(e)},i._formatSettingValue=function(e){return e?String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}}function Ba(i){i.renderRunProfilesPanel=function(e){let t=e.savedRunProfiles?.()??[],r=e.selectedRunProfile?.()??null,a=e.runProfileDraft?.()??{name:"",expose_as_button:!1},n=!!e.isRunProfileEditorOpen?.(),c=e.runProfileEditorMode?.()??"new";return`
          ...
          <section class="evcc-maintenance-panel evcc-maintenance-panel--wide">
            ...
            <div class="evcc-maintenance-tab-panel">
              ...
              ${p.length?`<div class="evcc-maintenance-card-grid">
                    ...
                    ${m==="maintenance_items"?this._renderStationWaterCard(_,b,r.station_water_label):""}
      ...
      <div class="evcc-modal-backdrop" data-action="close-maintenance-modal">
        <div class="evcc-modal evcc-maintenance-modal" data-stop-propagation>
          ...
          <div class="evcc-modal-body">
            ...
            ${n==="maintenance"?(()=>{let g=Number(r?.interval_hours),R=Number(r?.default_interval_hours),y=Number(r?.max_interval_hours),E=r?.reset_service_data?.vacuum_entity_id??"",M=r?.component??"",G=Number.isFinite(g)&&g>0?g:Number.isFinite(R)?R:"",z=[];return Number.isFinite(R)&&R>0&&z.push(`Default ${R}h`),Number.isFinite(y)&&y>0&&z.push(`Max ${y}h`),`
                <div class="evcc-editor-field-group">
    ...
    `},i._renderThemeTokenEditor=function(e,t){let r=this.card._state.getThemeGroupFilter(),a={},n=new Set;for(let o of se){let l=o.indexOf(" \u2014 ");if(l===-1)continue;let d=o.slice(0,l);se.includes(d)&&((a[d]=a[d]??[]).push(o),n.add(o))}let c=(o,l=!1)=>{let d=this.card._state.filteredThemeTokensForGroup(o,pe,{excludeKeys:et}),u=this.card._state.getThemeGroupSearchQuery(o),m=String(u||"").trim().length>0,p=r===o||m,f=(a[o]??[]).map(g=>c(g,!0)).filter(Boolean).join("");if(!d.length&&!p&&!f)return"";let h=this.card._state.themeGroupCounts(o,pe,{excludeKeys:et}),S=this.card._state.shouldForceThemeGroupOpenForSearch(o,pe,{excludeKeys:et})||this.card._state.isThemeGroupOpen(o),b=l?o.slice(o.lastIndexOf(" \u2014 ")+3):o;return`
        <div
    ...
    `}}function Fc(i){switch(i){case"cleaning":return"alert";case"returning":return"walking";case"paused":return"standing";case"error":return"warning";case"docked":case"idle":return"curled";default:return"curled"}}function ii(i){let e=0,t=0,r=0,a=i.length;for(let n=0,c=a-1;n<a;c=n++){let s=i[c][0]*i[n][1]-i[n][0]*i[c][1];e+=s,t+=(i[c][0]+i[n][0])*s,r+=(i[c][1]+i[n][1])*s}if(e*=.5,Math.abs(e)<1e-10){let n=i.reduce((s,o)=>s+o[0],0),c=i.reduce((s,o)=>s+o[1],0);return[n/a,c/a]}return[t/(6*e),r/(6*e)]}var rt=["#00e5ff","#ff6b35","#a3e635","#e879f9","#fbbf24","#a78bfa","#fb7185","#34d399","#60a5fa","#f472b6","#4ade80","#f97316"],Hc=[{key:"dark",label:"Dark",hint:"primary \u2014 clearest room colours"},{key:"light",label:"Light",hint:"assist \u2014 wall detection"},{key:"default",label:"Default",hint:"fallback"}];function ni(i){i.renderMapRoomView=function(t){let{state:r,vacuumStatus:a}=t,n=r.mapSegmentsData(),c=r.mapImageUrl(),s=!!r.liveMapImageEntity?.();if(!c||!n?.available&&!s){let g=(r.segmentationMode?.()??"cv")==="custom";return`
      ...
      `}let o=r.mapSegments(),l=r.selectedSegmentIds(),d=r.selectedSegments(),u=r.getRoomsForActiveMap?.()??[],m=o.map(g=>{let R=r.roomIdForSegment(g.segment_id),y=R!=null?u.find(E=>String(E.id)===String(R)):null;return typeof this._resolveSegmentFloorType=="function"?this._resolveSegmentFloorType(y):"default"}),p=r.mapZoom?.()??1,v=r.mapTranslateX?.()??0,f=r.mapTranslateY?.()??0,h=s?r.mapRotation?.()??0:0,_=r.canDrawZone?.()??!1,S=_&&(r.zoneDrawMode?.()??!1),b=r.zoneDraft?.()??null;return`
      <div class="evcc-map-view">
        <div class="evcc-map-container${S?" evcc-map-container--zone":""}">
          ...
          <div class="evcc-map-layers" style="transform:translate(${v}px,${f}px) scale(${p});transform-origin:0 0">
            ...
            >
              ...
              ${o.map((g,R)=>{let y=r.roomIdForSegment(g.segment_id),E=y!=null?u.find(z=>String(z.id)===String(y)):null,M=E?.name??g.name??g.label??`Segment ${g.segment_id}`,G=E?"Tap to queue \xB7 Double-tap to configure":"Tap to queue";return this._renderMapSegmentPolygon(g,l,R,M,G)}).join("")}
              ${typeof this._renderFloorTexturePolygon=="function"?o.map((g,R)=>this._renderFloorTexturePolygon(g,m[R])).join(""):""}
            </svg>
            ${this._renderMapAnimal(r,a)}
            ${r.mapRoomLabelsEnabled?.()??!0?o.map(g=>{let R=g.polygon_pct;if(!Array.isArray(R)||R.length<3)return"";let[y,E]=ii(R),M=Math.min(Math.max(y,5),95),G=Math.min(Math.max(E,6),94),z=r.roomIdForSegment(g.segment_id),D=(z!=null?u.find(ne=>String(ne.id)===String(z)):null)?.name??g.name??g.label??null;if(!D)return"";let me=d.findIndex(ne=>String(ne.segment_id)===String(g.segment_id)),re=me>=0;return`<div class="evcc-map-label${re?" evcc-map-label--selected":""}" style="left:${M}%;top:${G}%">
                ${re?`<span class="evcc-map-label-order">${me+1}</span>`:""}
                <span class="evcc-map-label-name">${this.escapeHtml(D)}</span>
              </div>`}).join(""):""}
            </div>
            ${S?`<div class="evcc-zone-draft" style="${b?`left:${Math.min(b.x,b.x+b.w)}%;top:${Math.min(b.y,b.y+b.h)}%;width:${Math.abs(b.w)}%;height:${Math.abs(b.h)}%;display:block`:"display:none"}"></div>`:""}
          ...
          <div class="evcc-zone-bar" role="group" aria-label="Zone clean controls">
            <span class="evcc-zone-bar-hint">${b?"Clean this box, or drag a new one":"Drag a box on the map"}</span>
            <button class="evcc-zone-bar-btn evcc-zone-bar-btn--primary"
                    data-action="zone-clean-confirm"${b?"":" disabled"}>Clean zone</button>
    ...
    />`},i._renderMapAnimal=function(t,r){if(!(t.mapAnimalEnabled?.()??!0))return"";let a=t.mapSegments(),n=t.getRoomsForActiveMap?.()??[],c=null,s=y=>{if(y==null)return null;let E=t.segmentIdForRoom?.(y);return E==null?null:a.find(M=>String(M.segment_id)===String(E))??null},o=r==="docked"||r==="idle";if(o){let y=n.find(E=>E.isDockRoom);c=s(y?.id)}if(!c){let y=t.dashboardJobProgress?.(),E=t.mascotDwelledRoomId?.()??y?.position_room_id??y?.current_room_id;c=s(E)}if(c||(c=a[0]??null),!c)return"";let l=t.roomIdForSegment(c.segment_id),d=o?"dock":l!=null?String(l):`seg_${c.segment_id}`,u,m,p=t.roomDotAnchor?.(d);if(p)u=p.pct_x,m=p.pct_y;else{let y=c.polygon_pct;if(!Array.isArray(y)||y.length<3)return"";[u,m]=ii(y)}let v=Fc(r??""),f=v==="curled",h=t.mapAnimalSelection?.()??"cat",_=t.mapAnimalScale?.()??1,S=t.batteryState?.()??"good",b=d,g=Math.round(64*_),R=Math.round(44*_);return`<div
    ...
    `},i._renderComposerRoomAssign=function(t){let r=t.composeSelectedId?.();if(r==null)return"";let a=t.composeDraft?.()??[],n=a.find(d=>d.id===r);if(!n)return"";let c=t.getRoomsForActiveMap?.()??[];if(!c.length)return`
      ...
      </div>`},i._renderVariantsSection=function(t,r,a,n){let c=n?.mapVariantDeleteArmed?.()??null,s=Hc.map(({key:p,label:v,hint:f})=>{let h=t[p],_=a?.type==="upload"&&a?.variant===p&&a?.status==="busy",S=a?.type==="analyze"&&a?.variant===p&&a?.status==="busy",b=_||S,g=a?.type==="upload"&&a?.variant===p&&a?.status==="error",R=h?`${h.width} \xD7 ${h.height}`:"not uploaded",y=h?"evcc-map-variant-status--ok":"evcc-map-variant-status--missing",E=_?"Uploading\u2026":S?"Analyzing\u2026 (10-30s)":"Upload";return`
        <div class="evcc-map-variant-row">
          <div class="evcc-map-variant-info">
    ...
    `}}function Mt(i){let e=String(i?.floor_type??"").toLowerCase().trim(),t=String(i?.carpet_type??"").toLowerCase().trim();return e==="carpet"?t==="high_pile"||t==="high"?"carpet_high":"carpet_low":e==="carpet_low_pile"||e==="carpet_low"?"carpet_low":e==="carpet_high_pile"||e==="carpet_high"?"carpet_high":e==="hardwood"||e==="laminate"||e==="wood"?"wood":e==="tile"?"tile":e==="marble"?"marble":e==="concrete"?"concrete":e==="granite"||e==="granite_light"?"granite_light":"default"}function zc(i){let e=String(i??""),t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619)>>>0;let r=t%101,a=(t>>>13^t>>>7)%101;return`${r}% ${a}%`}function ci(i){i._resolveSegmentFloorType=function(e){return Mt({floor_type:e?.floor_type??e?.floorType??"",carpet_type:e?.carpet_type??e?.carpetType??""})},i._renderFloorTextureLayer=function(e){if(!(this.card?._state?.roomFloorTextureEnabled?.()??!0))return"";let t=Mt({floor_type:e?.floorType??"",carpet_type:e?.carpetType??""}),r=Pe[t]??Pe.default,a=r.opacityDefault??.85,n=`var(--evcc-floor-${t}-opacity-card,var(--evcc-floor-texture-opacity-card,${a}))`,c=zc(e?.id??e?.name??t),s=r.layers.map(o=>{let l=`var(${o.colorToken},${o.colorDefault})`,d=`url(${o.url})`,u=`var(${o.opacityToken},${o.opacityDefault})`,m=`<span class="evcc-ftx-layer" data-role="${o.role}" style="background-color:${l};mask-image:${d};-webkit-mask-image:${d};mask-mode:luminance;-webkit-mask-source-type:luminance;--layer-opacity:${u}"></span>`;if(!o.blurToken&&!o.blurDefault)return m;let p=`var(${o.blurToken},${o.blurDefault??"0px"})`;return`<span class="evcc-ftx-blur" style="filter:blur(${p});-webkit-filter:blur(${p})">${m}</span>`}).join("");return`<div class="evcc-room-texture-layer" data-floor="${t}" style="--floor-opacity-card:${n};--floor-position-card:${c}">${s}</div>`},i._buildFloorTextureDefs=function(e){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let t=new Set,r=[];for(let a of e){if(t.has(a))continue;t.add(a);let n=Rt(a);n&&r.push(`<pattern id="evcc-ftx-${a}" patternUnits="userSpaceOnUse" width="8" height="8"><image href="${n}" width="8" height="8" preserveAspectRatio="xMidYMid slice"/></pattern>`)}return r.length?`<defs>${r.join("")}</defs>`:""},i._renderFloorTexturePolygon=function(e,t){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let r=e.polygon_pct;return!Array.isArray(r)||r.length<3||!Rt(t)?"":`<polygon class="evcc-map-texture-polygon" points="${r.map(([n,c])=>`${n},${c}`).join(" ")}" fill="url(#evcc-ftx-${t})" data-floor="${t}"/>`}}var Bc=[{value:"hardwood",label:"Hardwood"},{value:"laminate",label:"Laminate"},{value:"tile",label:"Tile"},{value:"marble",label:"Marble"},{value:"granite",label:"Granite"},{value:"concrete",label:"Concrete"},{value:"carpet_low_pile",label:"Low-Pile Carpet"},{value:"carpet_high_pile",label:"High-Pile Carpet"}];function oi(i){i.renderSetupView=function(e){let{state:t,card:r}=e,a=r._config?.vacuum_entity_id??"",n=t.setupStatus?.()??null,c=t.setupLoading?.()??!1,s=t.setupError?.()??null,o=t.setupLastResult?.()??null,l=Array.isArray(n?.vacuums)?n.vacuums:[],d=l.find(k=>k.vacuum_entity_id===a)??null,u=Array.isArray(d?.setup_steps)&&d.setup_steps.length?d.setup_steps:jc(d),m=d?.room_drift??null,p=t.setupRoomEditorOpenMapId?.()??null,v=t.setupRoomEditorLoadingMapId?.()??null,f=t.setupRoomEditorRooms?.()??[],h=t.setupRoomEditorSaving?.()??!1,_=t.setupDeletePendingMapId?.()??null,S=t.setupDeleteStage?.()??null,b=t.setupDeleteTypedToken?.()??"",g=t.setupDeleteDeleting?.()??!1,R=new Set((t.setupRoomEditorEnabledIds?.()??[]).map(String)),y=t.setupRoomEditorFloorTypesMap?.()??{},E=(d?.maps??[]).filter(k=>k.imported),M=c?'<div class="evcc-setup-result info">Working\u2026</div>':"",G=s&&!c?`<div class="evcc-setup-result error">${this.escapeHtml(String(s))}</div>`:"",z=(()=>{if(!o||c)return"";let k=o.status??"",B=o.message??"";return k==="error"||k==="blocked"?`<div class="evcc-setup-result error">${this.escapeHtml(B)}</div>`:B?`<div class="evcc-setup-result success">${this.escapeHtml(B)}</div>`:""})(),re={add_vacuum:k=>k.completed?`
      ...
      `},save_rooms:k=>{let B=u.find(j=>j.id==="import_active_map"),I=!!B,W=!B||B.completed;if(!si(u,"add_vacuum"))return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';if(I&&!W)return'<div class="evcc-setup-step-body muted">Complete map import first.</div>';if(E.length===0&&!I)return`
          <div class="evcc-setup-step-body">
            No rooms discovered yet. Run a clean cycle so the vacuum reports
            its room list, then refresh setup status.
          </div>
        `;let ae=ne(m,d),fe=E.map(j=>_e(j,!0)).join("");return`
      ...
      `}},ne=(k,B)=>{if(!k||k.in_sync)return"";let I=Array.isArray(k.new_rooms)?k.new_rooms:[],W=Array.isArray(k.removed_rooms)?k.removed_rooms:[],X=Array.isArray(k.transiently_missing)?k.transiently_missing:[];if(I.length===0&&W.length===0&&X.length===0)return"";let ae=I.length===0?"":`
      ...
      `},ge=k=>v===k?`<div class="evcc-setup-room-editor">
        ...
        <div class="evcc-setup-room-editor">
          ...
          <div class="evcc-setup-room-list">
            ${f.length===0?'<div class="evcc-setup-step-body muted">No rooms found for this map.</div>':f.map(I=>{let W=String(I.room_id),X=this.escapeHtml(I.name??`Room ${W}`),ae=R.has(W),fe=y[W]??"hardwood",Te=Bc.map(j=>`
      ...
      `,be=(k,B)=>{if(_!==k)return"";let I=this.escapeHtml(B?.typed_confirmation_value??`Map ${k}`),W=B?.requires_typed_confirmation??!1,X=B?.reasons??[],ae=X.length?`<div class="evcc-setup-delete-badges">
             ...
             <input class="evcc-setup-delete-input"
                    ...
                    autocomplete="off"
                    spellcheck="false" />
           </div>`:"",Te=W?b.trim()===(B?.typed_confirmation_value??"").trim():!0;return`
      ...
      `},_e=(k,B)=>{let I=String(k.map_id),W=this.escapeHtml(k.display_name??`Map ${I}`),X=t.isSetupMapConfigured?.(I),ae=p===I||v===I,fe=k.protection??null,Te=fe?.requires_typed_confirmation??!1,j=_===I,Dn=X&&!ae?'<span class="evcc-setup-configured-badge">\u2713 Configured</span>':"",Fn=B?`
      ...
      `},L=(k,B)=>{let I=re[k.id],W=I?I(k):`<div class="evcc-setup-step-body muted">No handler for step "${this.escapeHtml(k.id)}".</div>`,X=k.completed?"\u2713":String(B+1);return`
      ...
      <div class="evcc-setup-rename">
        ...
        <div class="evcc-setup-rename-row">
          <select class="evcc-setup-rename-input evcc-setup-map-camera-select"
            ...
            ${Re.map(k=>{let B=r._hass.states[k]?.attributes?.friendly_name??k,I=k===Q?" selected":"";return`<option value="${this.escapeHtml(k)}"${I}>${this.escapeHtml(String(B))} (${this.escapeHtml(k)})</option>`}).join("")}
          </select>
        </div>
      </div>
    ...
    `},i._renderJobBoundsEntry=function(e,t,r,a,n,c){let s=S=>Math.round(S).toLocaleString(),o=!!e.excluded,l=c.isMappingJobActionPending?.(r,t),d=[];if(!o){let S=n.filter((b,g)=>g!==t&&!b.excluded);if(S.length>0){let b={min_x:Math.min(...S.map(y=>y.min_x)),max_x:Math.max(...S.map(y=>y.max_x)),min_y:Math.min(...S.map(y=>y.min_y)),max_y:Math.max(...S.map(y=>y.max_y))},g=(b.max_x-b.min_x)*.1,R=(b.max_y-b.min_y)*.1;e.max_x>b.max_x+g&&d.push("max X"),e.min_x<b.min_x-g&&d.push("min X"),e.max_y>b.max_y+R&&d.push("max Y"),e.min_y<b.min_y-R&&d.push("min Y")}}let u=d.length>0,m=this._mrevFmtJobId(e.job_id),p=e.recorded_at?this._mrevFmtDate(e.recorded_at):"",v=t===n.length-1,f=n.filter(S=>!S.excluded).length,h=!o&&!l&&!v&&f>1,_=o&&!l&&!v;return`
    ...
    `},i._renderExtWizardStep1=function(e,t){return e.resegmentable?this._renderExtWizardStep1V2(e):this._renderExtWizardStep1Legacy(e,t)},i._segFacts=function(e){let t=e.settings||{};return`${(Number(e.area_m2)||0).toFixed(0)} m\xB2 \xB7 ${Math.round((Number(e.time_wall_s)||0)/60)} min \xB7 ${this.escapeHtml(String(t.clean_mode||"?"))} \xB7 ${e.pass_count||1}\xD7`},i._renderExtWizardStep1V2=function(e){let t=e.segments||[],r=t.length,a=Array.isArray(e.candidates)?e.candidates:[],n=a.length+1,c=!!e.busy,s=new Set((e.activeBoundaries||[]).map(Number)),o=a.filter(v=>!s.has(Number(v.id))),l=t.map((v,f)=>f===0?0:Number(v.boundary_id)),d=v=>v+1<t.length?l[v+1]:1/0,u=`
      ...
      </div>`,m=t.map((v,f)=>{let h=v.boundary_id,_=f===0?'<span class="evcc-ext-seg-start">First room</span>':`<button class="evcc-ext-merge" data-action="ext-merge-up" data-boundary-id="${h}" ${c?"disabled":""}>\u21A5 Merge up</button>`,b=o.filter(g=>Number(g.id)>l[f]&&Number(g.id)<d(f)).map(g=>`
    ...
    `},i._renderExtWizardStep2=function(e,t,r){return r.map((a,n)=>this._renderExtRoomPanel(e,t,a,n)).join("")},i._renderExtRoomPanel=function(e,t,r,a){let n=r.lead||{},c=Number(n.order??0),s=t.assignments[c]||{overrides:{}},o=(r.segments||[]).reduce((y,E)=>y+(Number(E.area_m2)||0),0),l=n.settings||{},d=s.overrides||{},m=(Array.isArray(n.shortlist)?n.shortlist:[]).map(y=>`
      ...
      </button>`).join(""),p=d.clean_mode??l.clean_mode,v=[["vacuum","Vacuum"],["vacuum_mop","Vac & Mop"],["mop","Mop"]].map(([y,E])=>`<button class="evcc-chip ${p===y?"active":""}"
      ...
      data-action="ext-set-override" data-order="${c}" data-key="clean_passes" data-value="${y}">${y}\xD7</button>`).join(""),_=p==="mop"||p==="vacuum_mop",S=y=>typeof e.state[y]=="function"?e.state[y]():[],b=this._extSettingRow(c,"Suction","fan_speed",S("suctionLevelOptions"),d.fan_speed??l.fan_speed),g=this._extSettingRow(c,"Cleaning Path","clean_intensity",S("cleanIntensityOptions"),d.clean_intensity??l.clean_intensity),R=_?this._extSettingRow(c,"Water","water_level",S("waterLevelOptions"),d.water_level??l.water_level):"";return`
    ...
    `,this.shadowRoot.getElementById("vacuum")?.addEventListener("change",c=>{this._fire({...this._config,vacuum_entity_id:c.target.value,room_id:void 0})}),this.shadowRoot.getElementById("room")?.addEventListener("change",c=>{let s=c.target.value,o=Number(s);this._fire({...this._config,room_id:Number.isFinite(o)?o:s})}),this.shadowRoot.getElementById("name")?.addEventListener("change",c=>{let s=c.target.value.trim(),o={...this._config};s?o.name=s:delete o.name,this._fire(o)})}static getConfigElement(){return document.createElement(Dt)}static getStubConfig(e){let t=e?.states??{},r=Object.keys(t).find(n=>n.startsWith("vacuum."))??"",a=Object.entries(t).find(([n,c])=>n.startsWith("switch.")&&c.attributes?.vacuum_entity_id===r&&c.attributes?.room_id!=null);return{vacuum_entity_id:r,room_id:a?.[1]?.attributes?.room_id??null}}};customElements.define(Dt,Pt);var Nt=class extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._hass=null,this._config=null,this._fields=null,this._saving=!1,this._starting=!1}setConfig(e){this._config=e??{},this._fields=null,this._render()}set hass(e){this._hass=e,this._render()}_objectId(){return(this._config?.vacuum_entity_id??"").split(".")[1]??""}_allRoomSwitches(){let{states:e}=this._hass??{},t=this._config?.vacuum_entity_id;return!e||!t?[]:Object.entries(e).filter(([r,a])=>r.startsWith("switch.")&&a.attributes?.vacuum_entity_id===t&&a.attributes?.room_id!=null).map(([r,a])=>({entityId:r,state:a.state,attrs:a.attributes??{}}))}_targetSwitch(){let e=String(this._config?.room_id??"");return this._allRoomSwitches().find(t=>String(t.attrs.room_id)===e)??null}_adapterOptions(e){let r=this._targetSwitch()?.attrs?.[e];return Array.isArray(r)?r:[]}_cleanModeOptions(){return this._adapterOptions("clean_mode_options")}_suctionOptions(){return this._adapterOptions("fan_speed_options")}_waterLevelOptions(){return this._adapterOptions("water_level_options")}_cleanIntensityOptions(e,t){return this._adapterOptions("clean_intensity_options")}_isMopMode(e){return String(e??"").toLowerCase().replace(/[\s_-]/g,"").includes("mop")}_committedFields(){let e=this._targetSwitch()?.attrs??{};return{clean_mode:e.clean_mode??"vacuum",fan_speed:e.fan_speed??null,water_level:e.water_level??null,clean_intensity:e.clean_intensity??null,clean_passes:Number(e.clean_passes??1),edge_mopping:!!(e.edge_mopping??!1)}}_currentFields(){return this._fields??this._committedFields()}_isDirty(){if(!this._fields)return!1;let e=this._committedFields();return Object.keys(this._fields).some(t=>this._fields[t]!==e[t])}_setField(e,t){this._fields={...this._currentFields(),[e]:t},this._render()}_render(){if(!this._config?.vacuum_entity_id||this._config?.room_id==null){this.shadowRoot.innerHTML="";return}let e=this._targetSwitch(),t=e?.attrs??{},r=e?.state==="on",a=this._config.name??t.room_name??`Room ${this._config.room_id}`,n=t.slug??"",c=String(t.map_id??""),s=!!(t.carpet??!1),o=this._currentFields(),l=this._isDirty(),d=this._isMopMode(o.clean_mode),u=this._cleanModeOptions(),m=this._suctionOptions(),p=d&&!s?this._waterLevelOptions():[],v=this._cleanIntensityOptions(n,c),f=d&&!s,h=(g,R,y,E)=>y.length?`
        <div class="field-group">
          ...
          <div class="chips">
            ${y.map(M=>`
              <button
                class="chip ${String(E??"").toLowerCase()===String(M.value??"").toLowerCase()?"active":""}"
                data-field="${ve(R)}"
