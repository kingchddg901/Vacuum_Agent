...
config/eufy_vacuum/battery/${this.escapeHtml(w)}/samples.jsonl</pre>
  ...
  `},i.renderRoomCard=function(e,t){let r=this._normalizeRoomDisplayData(e),a=r.cleanMode?r.cleanModeLabel||this._formatCleanMode(r.cleanMode):null,c=!r.fanSpeed||["off","normal"].includes(String(r.fanSpeed).toLowerCase())?null:r.fanSpeedLabel||this._formatFanSpeed(r.fanSpeed),o=!r.cleanIntensity||String(r.cleanIntensity).toLowerCase()==="standard"?null:r.cleanIntensityLabel||this._formatCleanIntensity(r.cleanIntensity),l=this._isMopMode(r.cleanMode)&&r.waterLevel&&String(r.waterLevel).toLowerCase()!=="off"?r.waterLevelLabel||this._formatWaterLevel(r.waterLevel):null,d=this._isMopMode(r.cleanMode)&&r.edgeMopping?"Edge Mop On":null,u=Number(r.cleanPasses)>1?`${Number(r.cleanPasses)}\xD7 passes`:null,m=this.card?._state?.orderDragItemId?.(),p=this.card?._state?.orderDragOverItemId?.(),v=String(m)===String(r.id)?"evcc-order-drag-source":"",f=String(p)===String(r.id)?"evcc-order-drag-target":"",h=t?.roomEstimateForRoom?.(r.id)??null,b=t?.dashboardPlannedWaterRoomForRoom?.(r.id,r.slug)??null,w="";if(h&&h.error==null){let L=h.source==="learned"?"evcc-room-status--estimate-learned":"evcc-room-status--estimate-default",T=h.source==="learned"?this._formatLearningMinutes(h.minutes):`~${this._formatLearningMinutes(h.minutes)}`,G=[`Estimate: ${this._formatLearningMinutes(h.minutes)}`];h.source&&G.push(`Source: ${String(h.source)}`);let Y=Number(h.battery);Number.isFinite(Y)&&G.push(`Battery: ${Y}`);let J=G.join(" \xB7 ");w=`
    ...
    `}let S="",g="",R="";if(h&&h.error==null&&typeof this.renderConfidenceChip=="function")if(h.source==="learned"){let L=h?.confidence_breakpoint?.ui_variant,T=L==="success"?"Reliable":L==="warning"?"Learning":L==="error"?"Uncertain":null;T&&(S=this.renderConfidenceChip(h.confidence_breakpoint,T,T),L==="success"?g="evcc-room-card--confidence-high":L==="warning"?g="evcc-room-card--confidence-medium":g="evcc-room-card--confidence-low")}else h.source==="default"&&(S=this.renderConfidenceChip({ui_variant:"neutral"},"Unlearned","Unlearned"));let _=String(b?.effective_clean_mode??b?.clean_mode??"").toLowerCase(),k=String(b?.effective_water_level??b?.water_level??"").toLowerCase();if(!!(b?.mop_active||this._isMopMode(_))&&k!=="off"){let L=Number(b.estimated_robot_water_used_ml);Number.isFinite(L)&&(R=`
        <div
  ...
  `},i._normalizeRoomDisplayData=function(e){let t=e?.selected_profile_details??{},r=String(e?.profile_name??e?.profileName??e?.profile??"vacuum_quick"),a=String(e?.clean_mode??e?.cleanMode??t?.clean_mode??"vacuum"),n=String(e?.fan_speed??e?.fanSpeed??t?.fan_speed??""),c=String(e?.water_level??e?.waterLevel??t?.water_level??""),s=String(e?.clean_intensity??e?.cleanIntensity??t?.clean_intensity??""),o=Number(e?.clean_passes??e?.cleanPasses??e?.passes??t?.default_clean_passes??1),l=!!(e?.edge_mopping??e?.edgeMopping??t?.default_edge_mopping??!1),d=String(e?.floor_type??e?.floorType??""),u=String(e?.carpet_type??e?.carpetType??""),m=!!(e?.carpet??(()=>{let v=String(d).toLowerCase();return v==="carpet"||v.startsWith("carpet_")||v.startsWith("carpet-")})()),p=Number(e?.order??e?.displayOrder??e?.position??999999);return{id:e?.id,mapId:e?.mapId??e?.map_id??"",name:e?.name??e?.room_name??"",slug:e?.slug??e?.room_slug??null,enabled:!!e?.enabled,order:Number.isFinite(p)?p:999999,profileName:r,profileLabel:e?.profile_label??e?.profileLabel??e?.selected_profile_label??e?.resolved_profile_label??null,profileSubtitle:e?.profile_subtitle??e?.profileSubtitle??null,lastCleanedAt:e?.lastCleanedAt??e?.last_cleaned_at??null,lastJobMode:e?.lastJobMode??e?.last_job_mode??null,isCustomProfile:r.toLowerCase()==="custom",cleanMode:a,cleanModeLabel:e?.clean_mode_label??e?.cleanModeLabel??t?.clean_mode_label??null,fanSpeed:n,fanSpeedLabel:e?.fan_speed_label??e?.fanSpeedLabel??t?.fan_speed_label??null,waterLevel:c,waterLevelLabel:e?.water_level_label??e?.waterLevelLabel??t?.water_level_label??null,cleanIntensity:s,cleanIntensityLabel:e?.clean_intensity_label??e?.cleanIntensityLabel??t?.clean_intensity_label??t?.path_type_label??null,cleanPasses:Number.isFinite(o)?o:1,cleanPassesLabel:e?.clean_passes_label??e?.cleanPassesLabel??t?.clean_passes_label??null,edgeMopping:l,edgeMoppingLabel:e?.edge_mopping_label??e?.edgeMoppingLabel??t?.edge_mopping_label??null,floorType:d,floorTypeLabel:e?.floor_type_label??e?.floorTypeLabel??null,carpetType:u,carpetTypeLabel:e?.carpet_type_label??e?.carpetTypeLabel??null,carpet:m,selectedProfileDetails:t}},i._isMopMode=function(e){let t=String(e??"").toLowerCase();return t==="mop"||t==="vacuum_mop"||t.includes("mop")||t.includes("wash")},i._roomProfileLabel=function(e){let t=String(e??"").trim();return t?t.toLowerCase()==="custom"?"Custom":t==="vacuum_quick"?"Vacuum Only Quick":t==="vacuum_deep"?"Vacuum Only Deep":t==="vacuum_mop_quick"?"Quick":t==="vacuum_mop_deep"?"Deep":t==="user_1"?"User Profile 1":t.replace(/[_-]+/g," ").replace(/\b\w/g,r=>r.toUpperCase()):"Standard"},i._formatCleanMode=function(e){let t=String(e??"").trim().toLowerCase();return t==="vacuum_mop"||t==="vacuum and mop"?"Vacuum + Mop":t==="vacuum"?"Vacuum":t==="mop"?"Mop":this._formatSettingValue(e)},i._formatFanSpeed=function(e){return this._formatSettingValue(e)},i._formatWaterLevel=function(e){return this._formatSettingValue(e)},i._formatCleanIntensity=function(e){return this._formatSettingValue(e)},i._formatFloorType=function(e){return this._formatSettingValue(e)},i._formatSettingValue=function(e){return e?String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}}function za(i){i.renderRunProfilesPanel=function(e){let t=e.savedRunProfiles?.()??[],r=e.selectedRunProfile?.()??null,a=e.runProfileDraft?.()??{name:"",expose_as_button:!1},n=!!e.isRunProfileEditorOpen?.(),c=e.runProfileEditorMode?.()??"new";return`
      ...
      <div class="evcc-modal-backdrop" data-action="close-maintenance-modal">
        <div class="evcc-modal evcc-maintenance-modal" data-stop-propagation>
          ...
          <div class="evcc-modal-body">
            ...
            ${n==="maintenance"?(()=>{let g=Number(r?.interval_hours),R=Number(r?.default_interval_hours),_=Number(r?.max_interval_hours),k=r?.reset_service_data?.vacuum_entity_id??"",M=r?.component??"",W=Number.isFinite(g)&&g>0?g:Number.isFinite(R)?R:"",V=[];return Number.isFinite(R)&&R>0&&V.push(`Default ${R}h`),Number.isFinite(_)&&_>0&&V.push(`Max ${_}h`),`
                <div class="evcc-editor-field-group">
    ...
    `},i._renderThemeTokenEditor=function(e,t){let r=this.card._state.getThemeGroupFilter(),a={},n=new Set;for(let o of ne){let l=o.indexOf(" \u2014 ");if(l===-1)continue;let d=o.slice(0,l);ne.includes(d)&&((a[d]=a[d]??[]).push(o),n.add(o))}let c=(o,l=!1)=>{let d=this.card._state.filteredThemeTokensForGroup(o,pe,{excludeKeys:et}),u=this.card._state.getThemeGroupSearchQuery(o),m=String(u||"").trim().length>0,p=r===o||m,f=(a[o]??[]).map(g=>c(g,!0)).filter(Boolean).join("");if(!d.length&&!p&&!f)return"";let h=this.card._state.themeGroupCounts(o,pe,{excludeKeys:et}),w=this.card._state.shouldForceThemeGroupOpenForSearch(o,pe,{excludeKeys:et})||this.card._state.isThemeGroupOpen(o),S=l?o.slice(o.lastIndexOf(" \u2014 ")+3):o;return`
        <div
    ...
    `}}function Fc(i){switch(i){case"cleaning":return"alert";case"returning":return"walking";case"paused":return"standing";case"error":return"warning";case"docked":case"idle":return"curled";default:return"curled"}}function ai(i){let e=0,t=0,r=0,a=i.length;for(let n=0,c=a-1;n<a;c=n++){let s=i[c][0]*i[n][1]-i[n][0]*i[c][1];e+=s,t+=(i[c][0]+i[n][0])*s,r+=(i[c][1]+i[n][1])*s}if(e*=.5,Math.abs(e)<1e-10){let n=i.reduce((s,o)=>s+o[0],0),c=i.reduce((s,o)=>s+o[1],0);return[n/a,c/a]}return[t/(6*e),r/(6*e)]}var rt=["#00e5ff","#ff6b35","#a3e635","#e879f9","#fbbf24","#a78bfa","#fb7185","#34d399","#60a5fa","#f472b6","#4ade80","#f97316"],Dc=[{key:"dark",label:"Dark",hint:"primary \u2014 clearest room colours"},{key:"light",label:"Light",hint:"assist \u2014 wall detection"},{key:"default",label:"Default",hint:"fallback"}];function ii(i){i.renderMapRoomView=function(t){let{state:r,vacuumStatus:a}=t,n=r.mapSegmentsData(),c=r.mapImageUrl(),s=!!r.liveMapImageEntity?.();if(!c||!n?.available&&!s){let b=(r.segmentationMode?.()??"cv")==="custom";return`
      ...
      `}let o=r.mapSegments(),l=r.selectedSegmentIds(),d=r.selectedSegments(),u=r.getRoomsForActiveMap?.()??[],m=o.map(b=>{let w=r.roomIdForSegment(b.segment_id),S=w!=null?u.find(g=>String(g.id)===String(w)):null;return typeof this._resolveSegmentFloorType=="function"?this._resolveSegmentFloorType(S):"default"}),p=r.mapZoom?.()??1,v=r.mapTranslateX?.()??0,f=r.mapTranslateY?.()??0,h=s?r.mapRotation?.()??0:0;return`
      <div class="evcc-map-view">
        <div class="evcc-map-container">
          ...
          <div class="evcc-map-layers" style="transform:translate(${v}px,${f}px) scale(${p});transform-origin:0 0">
            ...
            >
              ...
              ${o.map((b,w)=>{let S=r.roomIdForSegment(b.segment_id),g=S!=null?u.find(k=>String(k.id)===String(S)):null,R=g?.name??b.name??b.label??`Segment ${b.segment_id}`,_=g?"Tap to queue \xB7 Double-tap to configure":"Tap to queue";return this._renderMapSegmentPolygon(b,l,w,R,_)}).join("")}
              ${typeof this._renderFloorTexturePolygon=="function"?o.map((b,w)=>this._renderFloorTexturePolygon(b,m[w])).join(""):""}
            </svg>
            ${this._renderMapAnimal(r,a)}
            ${r.mapRoomLabelsEnabled?.()??!0?o.map(b=>{let w=b.polygon_pct;if(!Array.isArray(w)||w.length<3)return"";let[S,g]=ai(w),R=Math.min(Math.max(S,5),95),_=Math.min(Math.max(g,6),94),k=r.roomIdForSegment(b.segment_id),W=(k!=null?u.find(F=>String(F.id)===String(k)):null)?.name??b.name??b.label??null;if(!W)return"";let V=d.findIndex(F=>String(F.segment_id)===String(b.segment_id)),de=V>=0;return`<div class="evcc-map-label${de?" evcc-map-label--selected":""}" style="left:${R}%;top:${_}%">
    ...
    />`},i._renderMapAnimal=function(t,r){if(!(t.mapAnimalEnabled?.()??!0))return"";let a=t.mapSegments(),n=t.getRoomsForActiveMap?.()??[],c=null,s=_=>{if(_==null)return null;let k=t.segmentIdForRoom?.(_);return k==null?null:a.find(M=>String(M.segment_id)===String(k))??null},o=r==="docked"||r==="idle";if(o){let _=n.find(k=>k.isDockRoom);c=s(_?.id)}if(!c){let _=t.dashboardJobProgress?.(),k=t.mascotDwelledRoomId?.()??_?.position_room_id??_?.current_room_id;c=s(k)}if(c||(c=a[0]??null),!c)return"";let l=t.roomIdForSegment(c.segment_id),d=o?"dock":l!=null?String(l):`seg_${c.segment_id}`,u,m,p=t.roomDotAnchor?.(d);if(p)u=p.pct_x,m=p.pct_y;else{let _=c.polygon_pct;if(!Array.isArray(_)||_.length<3)return"";[u,m]=ai(_)}let v=Fc(r??""),f=v==="curled",h=t.mapAnimalSelection?.()??"cat",b=t.mapAnimalScale?.()??1,w=t.batteryState?.()??"good",S=d,g=Math.round(64*b),R=Math.round(44*b);return`<div
    ...
    ><animal-svg animal="${this.escapeHtml(h)}" pose="${this.escapeHtml(v)}" width="${g}px" height="${R}px" battery-state="${this.escapeHtml(w)}"></animal-svg></div>`},i._renderMapSelectionBar=function(t,r){let a=r.getRoomsForActiveMap?.()??[];return`<div class="evcc-map-selection-bar">${t.map((c,s)=>{let o=r.roomIdForSegment(c.segment_id),l=o!=null?a.find(m=>String(m.id)===String(o)):null,d=l?.name??c.name??c.label??`Segment ${c.segment_id}`,u=l?this._mapRoomSettingsSummary(l):"";return`
        <div
          class="evcc-map-chip"
    ...
    `}}function Mt(i){let e=String(i?.floor_type??"").toLowerCase().trim(),t=String(i?.carpet_type??"").toLowerCase().trim();return e==="carpet"?t==="high_pile"||t==="high"?"carpet_high":"carpet_low":e==="carpet_low_pile"||e==="carpet_low"?"carpet_low":e==="carpet_high_pile"||e==="carpet_high"?"carpet_high":e==="hardwood"||e==="laminate"||e==="wood"?"wood":e==="tile"?"tile":e==="marble"?"marble":e==="concrete"?"concrete":e==="granite"||e==="granite_light"?"granite_light":"default"}function Hc(i){let e=String(i??""),t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619)>>>0;let r=t%101,a=(t>>>13^t>>>7)%101;return`${r}% ${a}%`}function ni(i){i._resolveSegmentFloorType=function(e){return Mt({floor_type:e?.floor_type??e?.floorType??"",carpet_type:e?.carpet_type??e?.carpetType??""})},i._renderFloorTextureLayer=function(e){if(!(this.card?._state?.roomFloorTextureEnabled?.()??!0))return"";let t=Mt({floor_type:e?.floorType??"",carpet_type:e?.carpetType??""}),r=Pe[t]??Pe.default,a=r.opacityDefault??.85,n=`var(--evcc-floor-${t}-opacity-card,var(--evcc-floor-texture-opacity-card,${a}))`,c=Hc(e?.id??e?.name??t),s=r.layers.map(o=>{let l=`var(${o.colorToken},${o.colorDefault})`,d=`url(${o.url})`,u=`var(${o.opacityToken},${o.opacityDefault})`,m=`<span class="evcc-ftx-layer" data-role="${o.role}" style="background-color:${l};mask-image:${d};-webkit-mask-image:${d};mask-mode:luminance;-webkit-mask-source-type:luminance;--layer-opacity:${u}"></span>`;if(!o.blurToken&&!o.blurDefault)return m;let p=`var(${o.blurToken},${o.blurDefault??"0px"})`;return`<span class="evcc-ftx-blur" style="filter:blur(${p});-webkit-filter:blur(${p})">${m}</span>`}).join("");return`<div class="evcc-room-texture-layer" data-floor="${t}" style="--floor-opacity-card:${n};--floor-position-card:${c}">${s}</div>`},i._buildFloorTextureDefs=function(e){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let t=new Set,r=[];for(let a of e){if(t.has(a))continue;t.add(a);let n=Rt(a);n&&r.push(`<pattern id="evcc-ftx-${a}" patternUnits="userSpaceOnUse" width="8" height="8"><image href="${n}" width="8" height="8" preserveAspectRatio="xMidYMid slice"/></pattern>`)}return r.length?`<defs>${r.join("")}</defs>`:""},i._renderFloorTexturePolygon=function(e,t){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let r=e.polygon_pct;return!Array.isArray(r)||r.length<3||!Rt(t)?"":`<polygon class="evcc-map-texture-polygon" points="${r.map(([n,c])=>`${n},${c}`).join(" ")}" fill="url(#evcc-ftx-${t})" data-floor="${t}"/>`}}var zc=[{value:"hardwood",label:"Hardwood"},{value:"laminate",label:"Laminate"},{value:"tile",label:"Tile"},{value:"marble",label:"Marble"},{value:"granite",label:"Granite"},{value:"concrete",label:"Concrete"},{value:"carpet_low_pile",label:"Low-Pile Carpet"},{value:"carpet_high_pile",label:"High-Pile Carpet"}];function si(i){i.renderSetupView=function(e){let{state:t,card:r}=e,a=r._config?.vacuum_entity_id??"",n=t.setupStatus?.()??null,c=t.setupLoading?.()??!1,s=t.setupError?.()??null,o=t.setupLastResult?.()??null,l=Array.isArray(n?.vacuums)?n.vacuums:[],d=l.find(E=>E.vacuum_entity_id===a)??null,u=Array.isArray(d?.setup_steps)&&d.setup_steps.length?d.setup_steps:Bc(d),m=d?.room_drift??null,p=t.setupRoomEditorOpenMapId?.()??null,v=t.setupRoomEditorLoadingMapId?.()??null,f=t.setupRoomEditorRooms?.()??[],h=t.setupRoomEditorSaving?.()??!1,b=t.setupDeletePendingMapId?.()??null,w=t.setupDeleteStage?.()??null,S=t.setupDeleteTypedToken?.()??"",g=t.setupDeleteDeleting?.()??!1,R=new Set((t.setupRoomEditorEnabledIds?.()??[]).map(String)),_=t.setupRoomEditorFloorTypesMap?.()??{},k=(d?.maps??[]).filter(E=>E.imported),M=c?'<div class="evcc-setup-result info">Working\u2026</div>':"",W=s&&!c?`<div class="evcc-setup-result error">${this.escapeHtml(String(s))}</div>`:"",V=(()=>{if(!o||c)return"";let E=o.status??"",z=o.message??"";return E==="error"||E==="blocked"?`<div class="evcc-setup-result error">${this.escapeHtml(z)}</div>`:z?`<div class="evcc-setup-result success">${this.escapeHtml(z)}</div>`:""})(),ce={add_vacuum:E=>E.completed?`
      ...
      `},fe=E=>v===E?`<div class="evcc-setup-room-editor">
        ...
        <div class="evcc-setup-room-editor">
          ...
          <div class="evcc-setup-room-list">
            ${f.length===0?'<div class="evcc-setup-step-body muted">No rooms found for this map.</div>':f.map(I=>{let U=String(I.room_id),X=this.escapeHtml(I.name??`Room ${U}`),re=R.has(U),he=_[U]??"hardwood",Te=zc.map(B=>`
      ...
      `,ge=(E,z)=>{if(b!==E)return"";let I=this.escapeHtml(z?.typed_confirmation_value??`Map ${E}`),U=z?.requires_typed_confirmation??!1,X=z?.reasons??[],re=X.length?`<div class="evcc-setup-delete-badges">
             ...
             <input class="evcc-setup-delete-input"
                    ...
                    autocomplete="off"
                    spellcheck="false" />
           </div>`:"",Te=U?S.trim()===(z?.typed_confirmation_value??"").trim():!0;return`
    ...
    `},i._renderExtWizardStep2=function(e,t,r){return r.map((a,n)=>this._renderExtRoomPanel(e,t,a,n)).join("")},i._renderExtRoomPanel=function(e,t,r,a){let n=r.lead||{},c=Number(n.order??0),s=t.assignments[c]||{overrides:{}},o=(r.segments||[]).reduce((_,k)=>_+(Number(k.area_m2)||0),0),l=n.settings||{},d=s.overrides||{},m=(Array.isArray(n.shortlist)?n.shortlist:[]).map(_=>`
      <button class="evcc-chip ${s.room_id===_.room_id?"active":""}"
        ...
        data-action="ext-set-override" data-order="${c}" data-key="clean_mode" data-value="${_}">${k}</button>`).join(""),f=Number(d.clean_passes??n.pass_count??1),h=[1,2].map(_=>`<button class="evcc-chip ${f===_?"active":""}"
    ...
    `},i._extSettingRow=function(e,t,r,a,n){if(!Array.isArray(a)||!a.length)return"";let c=String(n??""),s=a.map(o=>{let l=String(o.value??"");return`<button class="evcc-chip ${c&&c.toLowerCase()===l.toLowerCase()?"active":""}"
      ...
      <select class="evcc-ext-allrooms" data-action="ext-pick-room-select" data-order="${t}">
        ...
        ${r.length?'<button class="evcc-btn evcc-btn-warn" data-action="ext-wizard-override">Keep anyway</button>':""}`),`<div class="evcc-modal-footer">${a}<div class="evcc-modal-footer-row">${n}${c}</div></div>`}}var y={ROOMS:"rooms",MAINTENANCE:"maintenance",BASE_STATION:"base_station",METRICS:"metrics",LEARNING_REVIEW:"learning_review",ROOM_RULES:"room_rules",THEME:"theme",MAPPING_ARCHIVE:"mapping",MAP_CONFIG:"map_config",MAPPING_REVIEW:"mapping_review",SETUP:"setup"},at=[y.ROOMS,y.MAINTENANCE,y.BASE_STATION,y.METRICS,y.LEARNING_REVIEW,y.ROOM_RULES,y.THEME,y.MAP_CONFIG,y.MAPPING_REVIEW,y.SETUP];function we(i,e){return i===y.BASE_STATION?e?.supportsBaseStation?.()!==!1:i===y.MAPPING_REVIEW?e?.supportsMapBounds?.()!==!1:!0}function ui(i){let e=i._state,t=i._renderers;return{card:i,state:e,renderers:t,vacuumName:e.vacuumDisplayName(),vacuumStatus:e.vacuumState()??"unknown",vacuumStatusLabel:e.vacuumStateLabel?.()??null,dockStatus:e.dockStatus?.()??null,dockStatusLabel:e.dockStatusLabel?.()??null,battery:e.batteryLevel(),view:i._view??y.ROOMS}}function qc(i){return{cleaning:"cleaning",docked:"docked",returning:"returning",error:"error",paused:"paused"}[i]||""}function di(i){return String(i??"").replace(/[_-]+/g," ").replace(/\s+/g," ").trim().replace(/\w\S*/g,e=>e.charAt(0).toUpperCase()+e.slice(1).toLowerCase())}function Gc(i){let e=String(i??"").trim().toLowerCase();return{cleaning:"cleaning",washing:"cleaning",drying:"returning",emptying:"returning",charging:"charging",error:"error",fault:"error",offline:"offline",unavailable:"unavailable",idle:"docked",standby:"docked"}[e]||""}function mi(i){let{state:e,renderers:t,vacuumName:r,vacuumStatus:a,vacuumStatusLabel:n,dockStatus:c,dockStatusLabel:s,battery:o,view:l}=i,d=o!=null?`${o}%`:"",u=n??di(a),m=s??(c?di(c):"");return`
    ...
    <div class="evcc-nav">
      ...
      <button class="evcc-nav-tab ${l===y.ROOMS?"active":""}"
              data-view="${y.ROOMS}">
        Rooms
      </button>

      <button class="evcc-nav-tab ${l===y.MAINTENANCE?"active":""}"
              data-view="${y.MAINTENANCE}">
        Maintenance
      </button>

      ${we(y.BASE_STATION,e)?`
      <button class="evcc-nav-tab ${l===y.BASE_STATION?"active":""}"
              data-view="${y.BASE_STATION}">
        Base Station
      </button>
      `:""}

      <button class="evcc-nav-tab ${l===y.METRICS?"active":""}"
              data-view="${y.METRICS}">
        Metrics
      </button>

      <button class="evcc-nav-tab ${l===y.LEARNING_REVIEW?"active":""}"
              data-view="${y.LEARNING_REVIEW}">
        Learning Review
      </button>

      <button class="evcc-nav-tab ${l===y.ROOM_RULES?"active":""}"
              data-view="${y.ROOM_RULES}">
        Room Rules
      </button>

      <button class="evcc-nav-tab ${l===y.THEME?"active":""}"
              data-view="${y.THEME}">
        Theme
      </button>

      ${we(y.MAPPING_REVIEW,e)?`
      <button class="evcc-nav-tab ${l===y.MAPPING_REVIEW?"active":""}"
              data-view="${y.MAPPING_REVIEW}">
        Map Bounds
      </button>
      `:""}

      <button class="evcc-nav-tab ${l===y.SETUP?"active":""}"
              data-view="${y.SETUP}">
        Setup
      </button>

    </div>
  `}function pi(i){let{view:e,renderers:t}=i;switch(e){case y.ROOMS:return t.renderRoomsView?.(i)??'<div class="evcc-empty">Rooms view unavailable</div>';case y.MAINTENANCE:return t.renderMaintenanceView?.(i)??'<div class="evcc-empty">Maintenance view unavailable</div>';case y.BASE_STATION:return t.renderBaseStationView?.(i)??'<div class="evcc-empty">Base station view unavailable</div>';case y.METRICS:return t.renderMetricsView?.(i)??'<div class="evcc-empty">Metrics view unavailable</div>';case y.LEARNING_REVIEW:return t.renderLearningReviewView?.(i)??'<div class="evcc-empty">Learning review view unavailable</div>';case y.ROOM_RULES:return t.renderRoomRulesView?.(i)??'<div class="evcc-empty">Room rules view unavailable</div>';case y.THEME:return t.renderThemeView?.(i)??'<div class="evcc-empty">Theme view unavailable</div>';case y.MAP_CONFIG:return t.renderMapConfigView?.(i)??'<div class="evcc-empty">Map config unavailable</div>';case y.MAPPING_REVIEW:return t.renderMappingReviewView?.(i)??'<div class="evcc-empty">Mapping bounds review unavailable</div>';case y.SETUP:return t.renderSetupView?.(i)??'<div class="evcc-empty">Setup unavailable</div>';default:return'<div class="evcc-empty">Unknown view</div>'}}var Uc=[{id:y.ROOMS,label:"Rooms",icon:Kc()},{id:y.MAINTENANCE,label:"Upkeep",icon:Yc()},{id:y.BASE_STATION,label:"Dock",icon:Qc()},{id:y.METRICS,label:"Stats",icon:Xc()}],vi=[{id:y.LEARNING_REVIEW,label:"Learning Review"},{id:y.ROOM_RULES,label:"Room Rules"},{id:y.THEME,label:"Theme"},{id:y.MAP_CONFIG,label:"Map Config"},{id:y.MAPPING_REVIEW,label:"Map Bounds"},{id:y.SETUP,label:"Setup"}];function Wc(i){return{cleaning:"cleaning",docked:"docked",returning:"returning",error:"error",paused:"paused"}[i]||""}function hi(i){return String(i??"").replace(/[_-]+/g," ").replace(/\s+/g," ").trim().replace(/\w\S*/g,e=>e.charAt(0).toUpperCase()+e.slice(1).toLowerCase())}function Jc(i){let e=String(i??"").trim().toLowerCase();return{cleaning:"cleaning",washing:"cleaning",drying:"returning",emptying:"returning",charging:"charging",error:"error",fault:"error",offline:"offline",unavailable:"unavailable",idle:"docked",standby:"docked"}[e]||""}function fi(i){i.renderMobileHeader=function(e){let{vacuumName:t,vacuumStatus:r,vacuumStatusLabel:a,dockStatus:n,dockStatusLabel:c,battery:s}=e,o=s!=null?`${s}%`:"",l=a??hi(r),d=c??(n?hi(n):"");return`
