...
config/eufy_vacuum/battery/${this.escapeHtml(w)}/samples.jsonl</pre>
    ...
    `}}function Kr(i){i.renderLearningReviewView=function(e){let{state:t}=e,r=this._renderReviewSubtabStrip(t),a=t.reviewSubtab()==="external"?this.renderExternalJobsSubtab(e):this._renderLearningHistoryView(e);return`<div class="evcc-review-shell">${r}${a}</div>`},i._renderLearningHistoryView=function(e){let{state:t}=e,r=t.learningHistorySnapshot?.();if(!r)return'<div class="evcc-empty">Loading learning history...</div>';if(r.available===!1)return`
  ...
  `},i._normalizeRoomDisplayData=function(e){let t=e?.selected_profile_details??{},r=String(e?.profile_name??e?.profileName??e?.profile??"vacuum_quick"),a=String(e?.clean_mode??e?.cleanMode??t?.clean_mode??"vacuum"),n=String(e?.fan_speed??e?.fanSpeed??t?.fan_speed??""),c=String(e?.water_level??e?.waterLevel??t?.water_level??""),s=String(e?.clean_intensity??e?.cleanIntensity??t?.clean_intensity??""),o=Number(e?.clean_passes??e?.cleanPasses??e?.passes??t?.default_clean_passes??1),l=!!(e?.edge_mopping??e?.edgeMopping??t?.default_edge_mopping??!1),d=String(e?.floor_type??e?.floorType??""),u=String(e?.carpet_type??e?.carpetType??""),m=!!(e?.carpet??(()=>{let p=String(d).toLowerCase();return p==="carpet"||p.startsWith("carpet_")||p.startsWith("carpet-")})()),v=Number(e?.order??e?.displayOrder??e?.position??999999);return{id:e?.id,mapId:e?.mapId??e?.map_id??"",name:e?.name??e?.room_name??"",slug:e?.slug??e?.room_slug??null,enabled:!!e?.enabled,order:Number.isFinite(v)?v:999999,profileName:r,profileLabel:e?.profile_label??e?.profileLabel??e?.selected_profile_label??e?.resolved_profile_label??null,profileSubtitle:e?.profile_subtitle??e?.profileSubtitle??null,lastCleanedAt:e?.lastCleanedAt??e?.last_cleaned_at??null,lastJobMode:e?.lastJobMode??e?.last_job_mode??null,isCustomProfile:r.toLowerCase()==="custom",cleanMode:a,cleanModeLabel:e?.clean_mode_label??e?.cleanModeLabel??t?.clean_mode_label??null,fanSpeed:n,fanSpeedLabel:e?.fan_speed_label??e?.fanSpeedLabel??t?.fan_speed_label??null,waterLevel:c,waterLevelLabel:e?.water_level_label??e?.waterLevelLabel??t?.water_level_label??null,cleanIntensity:s,cleanIntensityLabel:e?.clean_intensity_label??e?.cleanIntensityLabel??t?.clean_intensity_label??t?.path_type_label??null,cleanPasses:Number.isFinite(o)?o:1,cleanPassesLabel:e?.clean_passes_label??e?.cleanPassesLabel??t?.clean_passes_label??null,edgeMopping:l,edgeMoppingLabel:e?.edge_mopping_label??e?.edgeMoppingLabel??t?.edge_mopping_label??null,floorType:d,floorTypeLabel:e?.floor_type_label??e?.floorTypeLabel??null,carpetType:u,carpetTypeLabel:e?.carpet_type_label??e?.carpetTypeLabel??null,carpet:m,selectedProfileDetails:t}},i._isMopMode=function(e){let t=String(e??"").toLowerCase();return t==="mop"||t==="vacuum_mop"||t.includes("mop")||t.includes("wash")},i._roomProfileLabel=function(e){let t=String(e??"").trim();return t?t.toLowerCase()==="custom"?"Custom":t==="vacuum_quick"?"Vacuum Only Quick":t==="vacuum_deep"?"Vacuum Only Deep":t==="vacuum_mop_quick"?"Quick":t==="vacuum_mop_deep"?"Deep":t==="user_1"?"User Profile 1":t.replace(/[_-]+/g," ").replace(/\b\w/g,r=>r.toUpperCase()):"Standard"},i._formatCleanMode=function(e){let t=String(e??"").trim().toLowerCase();return t==="vacuum_mop"||t==="vacuum and mop"?"Vacuum + Mop":t==="vacuum"?"Vacuum":t==="mop"?"Mop":this._formatSettingValue(e)},i._formatFanSpeed=function(e){return this._formatSettingValue(e)},i._formatWaterLevel=function(e){return this._formatSettingValue(e)},i._formatCleanIntensity=function(e){return this._formatSettingValue(e)},i._formatFloorType=function(e){return this._formatSettingValue(e)},i._formatSettingValue=function(e){return e?String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}}function Qr(i){i.renderRunProfilesPanel=function(e){let t=e.savedRunProfiles?.()??[],r=e.selectedRunProfile?.()??null,a=e.runProfileDraft?.()??{name:"",expose_as_button:!1},n=!!e.isRunProfileEditorOpen?.(),c=e.runProfileEditorMode?.()??"new";return`
    ...
    `}}function ta(i){i.renderRoomEditorModal=function(e){let{state:t}=e;if(!t.isRoomEditorOpen())return"";let r=t.activeEditorRoom(),a=t.editorFields();if(!r||!a)return"";let n=t.isEditorRoomCarpet();return`
    ...
    `}}var gn=new Set(["is_on","is_off","exists","missing"]);function ra(i){i.renderRoomRulesView=function(e){let{state:t}=e,r=t.getRoomsForActiveMap?.()??[];if(!r.length)return`
    ...
    `},i._renderRuleEditor=function(e,t,r,a,n){let c=a==="new",s=r.kind==="modifier",o=e.ruleEntityDescriptor?.(r)??null,l=e.ruleOperatorGroups?.(r)??[],d=e.ruleEntitySearchResults?.(r.entity_id,10)??[],u=gn.has(r.operator??""),m=e.roomRulesDraftIsValid?.()??!1;return`
    ...
    `},i._renderThemeTokenRow=function(e,t,r){let a=r==="draft",n=t||"";return e.type==="color"?la(n)?this._renderThemeColorMixTokenRow(e,n,a):this._renderThemeColorTokenRow(e,n,a):En(e,n)?this._renderThemeNumericTokenRow(e,n,a):this._renderThemeTextTokenRow(e,n,a)},i._renderThemeColorTokenRow=function(e,t,r){let a=String(t||"").trim(),n=this._safeColorInputValue(a),c=Tn(a),s=/^#[0-9a-fA-F]{8}$/.test(a)?`#${a.slice(1,7)}`:a;return`
    ...
    `},i._renderThemeColorMixTokenRow=function(e,t,r){let a=la(t);if(!a)return this._renderThemeColorTokenRow(e,t,r);let{color1:n,ratio:c,color2:s}=a,o=this.escapeHtml(xn(n,c,s));return`
    ...
    `},i._renderThemeNumericTokenRow=function(e,t,r){let a=wn[e.group]||{min:0,max:64,step:1},n={min:Number.isFinite(e.min)?e.min:a.min,max:Number.isFinite(e.max)?e.max:a.max,step:Number.isFinite(e.step)?e.step:a.step},c=da(e,t),s=c.numeric??n.min,o=c.unit||Be(e),l=e.type==="number"?"":o,d=Math.min(n.min,s),u=Math.max(n.max,s);return`
      <div
    ...
    `}}function Cn(i){switch(i){case"cleaning":return"alert";case"returning":return"walking";case"paused":return"standing";case"error":return"warning";case"docked":case"idle":return"curled";default:return"curled"}}function pa(i){let e=0,t=0,r=0,a=i.length;for(let n=0,c=a-1;n<a;c=n++){let s=i[c][0]*i[n][1]-i[n][0]*i[c][1];e+=s,t+=(i[c][0]+i[n][0])*s,r+=(i[c][1]+i[n][1])*s}if(e*=.5,Math.abs(e)<1e-10){let n=i.reduce((s,o)=>s+o[0],0),c=i.reduce((s,o)=>s+o[1],0);return[n/a,c/a]}return[t/(6*e),r/(6*e)]}var je=["#00e5ff","#ff6b35","#a3e635","#e879f9","#fbbf24","#a78bfa","#fb7185","#34d399","#60a5fa","#f472b6","#4ade80","#f97316"],On=[{key:"dark",label:"Dark",hint:"primary \u2014 clearest room colours"},{key:"light",label:"Light",hint:"assist \u2014 wall detection"},{key:"default",label:"Default",hint:"fallback"}];function ha(i){i.renderMapRoomView=function(t){let{state:r,vacuumStatus:a}=t,n=r.mapSegmentsData(),c=r.mapImageUrl();if(!n?.available||!c)return`
      ...
      <div class="evcc-map-view">
        <div class="evcc-map-container">
          ...
          <div class="evcc-map-layers" style="transform:translate(${v}px,${p}px) scale(${m});transform-origin:0 0">
            ...
            ${s.map(f=>{let h=f.polygon_pct;if(!Array.isArray(h)||h.length<3)return"";let[b,w]=pa(h),S=Math.min(Math.max(b,5),95),g=Math.min(Math.max(w,6),94),R=r.roomIdForSegment(f.segment_id),k=(R!=null?d.find(q=>String(q.id)===String(R)):null)?.name??f.name??f.label??null;if(!k)return"";let M=l.findIndex(q=>String(q.segment_id)===String(f.segment_id)),J=M>=0;return`<div class="evcc-map-label${J?" evcc-map-label--selected":""}" style="left:${S}%;top:${g}%">
    ...
    ><animal-svg animal="${this.escapeHtml(h)}" pose="${this.escapeHtml(p)}" width="${g}px" height="${R}px" battery-state="${this.escapeHtml(w)}"></animal-svg></div>`},i._renderMapSelectionBar=function(t,r){let a=r.getRoomsForActiveMap?.()??[];return`<div class="evcc-map-selection-bar">${t.map((c,s)=>{let o=r.roomIdForSegment(c.segment_id),l=o!=null?a.find(m=>String(m.id)===String(o)):null,d=l?.name??c.name??c.label??`Segment ${c.segment_id}`,u=l?this._mapRoomSettingsSummary(l):"";return`
      ...
      <div class="evcc-map-config-view">
        ...
        <div class="evcc-map-config-panel">
          ${this._renderVariantsSection(l,d,u,r)}
        </div>
    ...
    `}}function ct(i){let e=String(i?.floor_type??"").toLowerCase().trim(),t=String(i?.carpet_type??"").toLowerCase().trim();return e==="carpet"?t==="high_pile"||t==="high"?"carpet_high":"carpet_low":e==="carpet_low_pile"||e==="carpet_low"?"carpet_low":e==="carpet_high_pile"||e==="carpet_high"?"carpet_high":e==="hardwood"||e==="laminate"||e==="wood"?"wood":e==="tile"?"tile":e==="marble"?"marble":e==="concrete"?"concrete":e==="granite"||e==="granite_light"?"granite_light":"default"}function Ln(i){let e=String(i??""),t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619)>>>0;let r=t%101,a=(t>>>13^t>>>7)%101;return`${r}% ${a}%`}function fa(i){i._resolveSegmentFloorType=function(e){return ct({floor_type:e?.floor_type??e?.floorType??"",carpet_type:e?.carpet_type??e?.carpetType??""})},i._renderFloorTextureLayer=function(e){let t=ct({floor_type:e?.floorType??"",carpet_type:e?.carpetType??""}),r=Re[t]??Re.default,a=r.opacityDefault??.85,n=`var(--evcc-floor-${t}-opacity-card,var(--evcc-floor-texture-opacity-card,${a}))`,c=Ln(e?.id??e?.name??t),s=r.layers.map(o=>{let l=`var(${o.colorToken},${o.colorDefault})`,d=`url(${o.url})`,u=`var(${o.opacityToken},${o.opacityDefault})`,m=`<span class="evcc-ftx-layer" data-role="${o.role}" style="background-color:${l};mask-image:${d};-webkit-mask-image:${d};mask-mode:luminance;-webkit-mask-source-type:luminance;--layer-opacity:${u}"></span>`;if(!o.blurToken&&!o.blurDefault)return m;let v=`var(${o.blurToken},${o.blurDefault??"0px"})`;return`<span class="evcc-ftx-blur" style="filter:blur(${v});-webkit-filter:blur(${v})">${m}</span>`}).join("");return`<div class="evcc-room-texture-layer" data-floor="${t}" style="--floor-opacity-card:${n};--floor-position-card:${c}">${s}</div>`},i._buildFloorTextureDefs=function(e){let t=new Set,r=[];for(let a of e){if(t.has(a))continue;t.add(a);let n=tt(a);n&&r.push(`<pattern id="evcc-ftx-${a}" patternUnits="userSpaceOnUse" width="8" height="8"><image href="${n}" width="8" height="8" preserveAspectRatio="xMidYMid slice"/></pattern>`)}return r.length?`<defs>${r.join("")}</defs>`:""},i._renderFloorTexturePolygon=function(e,t){let r=e.polygon_pct;return!Array.isArray(r)||r.length<3||!tt(t)?"":`<polygon class="evcc-map-texture-polygon" points="${r.map(([n,c])=>`${n},${c}`).join(" ")}" fill="url(#evcc-ftx-${t})" data-floor="${t}"/>`}}var Pn=[{value:"hardwood",label:"Hardwood"},{value:"laminate",label:"Laminate"},{value:"tile",label:"Tile"},{value:"marble",label:"Marble"},{value:"granite",label:"Granite"},{value:"concrete",label:"Concrete"},{value:"carpet_low_pile",label:"Low-Pile Carpet"},{value:"carpet_high_pile",label:"High-Pile Carpet"}];function ba(i){i.renderSetupView=function(e){let{state:t,card:r}=e,a=r._config?.vacuum_entity_id??"",n=t.setupStatus?.()??null,c=t.setupLoading?.()??!1,s=t.setupError?.()??null,o=t.setupLastResult?.()??null,d=(Array.isArray(n?.vacuums)?n.vacuums:[]).find(E=>E.vacuum_entity_id===a)??null,u=Array.isArray(d?.setup_steps)&&d.setup_steps.length?d.setup_steps:Nn(d),m=d?.room_drift??null,v=t.setupRoomEditorOpenMapId?.()??null,p=t.setupRoomEditorLoadingMapId?.()??null,f=t.setupRoomEditorRooms?.()??[],h=t.setupRoomEditorSaving?.()??!1,b=t.setupDeletePendingMapId?.()??null,w=t.setupDeleteStage?.()??null,S=t.setupDeleteTypedToken?.()??"",g=t.setupDeleteDeleting?.()??!1,R=new Set((t.setupRoomEditorEnabledIds?.()??[]).map(String)),_=t.setupRoomEditorFloorTypesMap?.()??{},k=(d?.maps??[]).filter(E=>E.imported),M=c?'<div class="evcc-setup-result info">Working\u2026</div>':"",J=s&&!c?`<div class="evcc-setup-result error">${this.escapeHtml(String(s))}</div>`:"",q=(()=>{if(!o||c)return"";let E=o.status??"",j=o.message??"";return E==="error"||E==="blocked"?`<div class="evcc-setup-result error">${this.escapeHtml(j)}</div>`:j?`<div class="evcc-setup-result success">${this.escapeHtml(j)}</div>`:""})(),ie={add_vacuum:E=>E.completed?`
      ...
      `},save_rooms:E=>{let j=u.find(V=>V.id==="import_active_map"),A=!!j,z=!j||j.completed;if(!ga(u,"add_vacuum"))return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';if(A&&!z)return'<div class="evcc-setup-step-body muted">Complete map import first.</div>';if(k.length===0&&!A)return`
          <div class="evcc-setup-step-body">
      ...
      `},he=E=>p===E?`<div class="evcc-setup-room-editor">
        ...
        <div class="evcc-setup-room-editor">
          ...
          <div class="evcc-setup-room-list">
            ${f.length===0?'<div class="evcc-setup-step-body muted">No rooms found for this map.</div>':f.map(A=>{let z=String(A.room_id),H=this.escapeHtml(A.name??`Room ${z}`),Z=R.has(z),ve=_[z]??"hardwood",ye=Pn.map(V=>`
      ...
      `},le=(E,j)=>{let A=String(E.map_id),z=this.escapeHtml(E.display_name??`Map ${A}`),H=t.isSetupMapConfigured?.(A),Z=v===A||p===A,ve=E.protection??null,ye=ve?.requires_typed_confirmation??!1,V=b===A,ji=H&&!Z?'<span class="evcc-setup-configured-badge">\u2713 Configured</span>':"",Vi=j?`
      ...
      `:"",qi=V?"":`<button class="evcc-setup-btn destructive-ghost small"
    ...
    `}}function ga(i,e){return Array.isArray(i)?!!i.find(r=>r.id===e)?.completed:!1}function Nn(i){if(!i)return[{id:"add_vacuum",label:"Add vacuum",completed:!1,service:""},{id:"import_active_map",label:"Import active map",completed:!1,service:""},{id:"save_rooms",label:"Configure rooms",completed:!1,service:""}];let e=!!i.has_imported_map;return[{id:"add_vacuum",label:"Add vacuum",completed:!0,service:""},{id:"import_active_map",label:"Import active map",completed:e,service:""},{id:"save_rooms",label:"Configure rooms",completed:e,service:""}]}var Fn="0 0 16 16",Hn=Object.freeze({ok:'<path d="M3.4 8.6l3 3 6.2-7" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>',outlier:'<path d="M4.4 4.4l7.2 7.2M11.6 4.4l-7.2 7.2" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/>',warn:'<path d="M8 3.2v5.6" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/><circle cx="8" cy="12.3" r="1.25" fill="currentColor"/>',likely:'<circle cx="8" cy="8" r="5.3" fill="none" stroke="currentColor" stroke-width="1.6"/><path d="M8 2.7a5.3 5.3 0 0 1 0 10.6z" fill="currentColor"/>',excluded:'<path d="M3.4 8h9.2" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round"/>',baseline:'<path d="M8 2.4l5.6 5.6-5.6 5.6L2.4 8z" fill="currentColor"/>'});function be(i){let e=Hn[i];return e?`<svg class="evcc-mrev-badge-mark" viewBox="${Fn}" aria-hidden="true" focusable="false">${e}</svg>`:""}function _a(i){i.renderMappingReviewView=function(e){let{state:t}=e,r=t.mappingBoundsSnapshot?.();if(!r)return'<div class="evcc-empty">Loading mapping bounds...</div>';if(r.available===!1)return`
    ...
    `},i._extSettingRow=function(e,t,r,a,n){if(!Array.isArray(a)||!a.length)return"";let c=String(n??""),s=a.map(o=>{let l=String(o.value??"");return`<button class="evcc-chip ${c&&c.toLowerCase()===l.toLowerCase()?"active":""}"
      ...
      <select class="evcc-ext-allrooms" data-action="ext-pick-room-select" data-order="${t}">
        ...
        ${r.length?'<button class="evcc-btn evcc-btn-warn" data-action="ext-wizard-override">Keep anyway</button>':""}`),`<div class="evcc-modal-footer">${a}<div class="evcc-modal-footer-row">${n}${c}</div></div>`}}var y={ROOMS:"rooms",MAINTENANCE:"maintenance",BASE_STATION:"base_station",METRICS:"metrics",LEARNING_REVIEW:"learning_review",ROOM_RULES:"room_rules",THEME:"theme",MAPPING_ARCHIVE:"mapping",MAP_CONFIG:"map_config",MAPPING_REVIEW:"mapping_review",SETUP:"setup"},Ve=[y.ROOMS,y.MAINTENANCE,y.BASE_STATION,y.METRICS,y.LEARNING_REVIEW,y.ROOM_RULES,y.THEME,y.MAP_CONFIG,y.MAPPING_REVIEW,y.SETUP];function wa(i){let e=i._state,t=i._renderers;return{card:i,state:e,renderers:t,vacuumName:e.vacuumDisplayName(),vacuumStatus:e.vacuumState()??"unknown",vacuumStatusLabel:e.vacuumStateLabel?.()??null,dockStatus:e.dockStatus?.()??null,dockStatusLabel:e.dockStatusLabel?.()??null,battery:e.batteryLevel(),view:i._view??y.ROOMS}}function Dn(i){return{cleaning:"cleaning",docked:"docked",returning:"returning",error:"error",paused:"paused"}[i]||""}function xa(i){return String(i??"").replace(/[_-]+/g," ").replace(/\s+/g," ").trim().replace(/\w\S*/g,e=>e.charAt(0).toUpperCase()+e.slice(1).toLowerCase())}function zn(i){let e=String(i??"").trim().toLowerCase();return{cleaning:"cleaning",washing:"cleaning",drying:"returning",emptying:"returning",charging:"charging",error:"error",fault:"error",offline:"offline",unavailable:"unavailable",idle:"docked",standby:"docked"}[e]||""}function Sa(i){let{renderers:e,vacuumName:t,vacuumStatus:r,vacuumStatusLabel:a,dockStatus:n,dockStatusLabel:c,battery:s,view:o}=i,l=s!=null?`${s}%`:"",d=a??xa(r),u=c??(n?xa(n):"");return`
    <div class="evcc-header">
      ...
      <div class="evcc-header-left">
        ...
        <div class="evcc-vacuum-status">
          <span class="evcc-status-dot ${Dn(r)}"></span>
  ...
  `}function Ra(i){let{view:e,renderers:t}=i;switch(e){case y.ROOMS:return t.renderRoomsView?.(i)??'<div class="evcc-empty">Rooms view unavailable</div>';case y.MAINTENANCE:return t.renderMaintenanceView?.(i)??'<div class="evcc-empty">Maintenance view unavailable</div>';case y.BASE_STATION:return t.renderBaseStationView?.(i)??'<div class="evcc-empty">Base station view unavailable</div>';case y.METRICS:return t.renderMetricsView?.(i)??'<div class="evcc-empty">Metrics view unavailable</div>';case y.LEARNING_REVIEW:return t.renderLearningReviewView?.(i)??'<div class="evcc-empty">Learning review view unavailable</div>';case y.ROOM_RULES:return t.renderRoomRulesView?.(i)??'<div class="evcc-empty">Room rules view unavailable</div>';case y.THEME:return t.renderThemeView?.(i)??'<div class="evcc-empty">Theme view unavailable</div>';case y.MAP_CONFIG:return t.renderMapConfigView?.(i)??'<div class="evcc-empty">Map config unavailable</div>';case y.MAPPING_REVIEW:return t.renderMappingReviewView?.(i)??'<div class="evcc-empty">Mapping bounds review unavailable</div>';case y.SETUP:return t.renderSetupView?.(i)??'<div class="evcc-empty">Setup unavailable</div>';default:return'<div class="evcc-empty">Unknown view</div>'}}var Bn=[{id:y.ROOMS,label:"Rooms",icon:qn()},{id:y.MAINTENANCE,label:"Upkeep",icon:Gn()},{id:y.BASE_STATION,label:"Dock",icon:Wn()},{id:y.METRICS,label:"Stats",icon:Un()}],Ea=[{id:y.LEARNING_REVIEW,label:"Learning Review"},{id:y.ROOM_RULES,label:"Room Rules"},{id:y.MAP_CONFIG,label:"Map Config"},{id:y.MAPPING_REVIEW,label:"Map Bounds"},{id:y.SETUP,label:"Setup"}];function jn(i){return{cleaning:"cleaning",docked:"docked",returning:"returning",error:"error",paused:"paused"}[i]||""}function ka(i){return String(i??"").replace(/[_-]+/g," ").replace(/\s+/g," ").trim().replace(/\w\S*/g,e=>e.charAt(0).toUpperCase()+e.slice(1).toLowerCase())}function Vn(i){let e=String(i??"").trim().toLowerCase();return{cleaning:"cleaning",washing:"cleaning",drying:"returning",emptying:"returning",charging:"charging",error:"error",fault:"error",offline:"offline",unavailable:"unavailable",idle:"docked",standby:"docked"}[e]||""}function Ta(i){i.renderMobileHeader=function(e){let{vacuumName:t,vacuumStatus:r,vacuumStatusLabel:a,dockStatus:n,dockStatusLabel:c,battery:s}=e,o=s!=null?`${s}%`:"",l=a??ka(r),d=c??(n?ka(n):"");return`
      <div class="evcc-mobile-header">
        ...
        ${d?`
          <div class="evcc-mobile-vacuum-status evcc-mobile-dock-status">
            <span class="evcc-status-dot ${Vn(n)}"></span>
            <span class="evcc-mobile-vacuum-status-label">
              <span class="evcc-status-prefix">Dock Status:</span>
              ${this.escapeHtml(d)}
            </span>
    ...
    `},i.renderMobileBottomNav=function(e){let t=Ea.some(r=>r.id===e);return`
  ...
  </svg>`}function $a(i){i.renderToasts=function(e){let t=e?.state?.activeToasts?.()??[];return t.length?`
...
`,mi=`
  ...
  ${di}
`;var vi=[qa,Ga,Wa,Ua,Ja,Ka,Ya,Qa,ot,lt,Xa,Za,ei,ti,ri,ai,ii,ni,ci,si,oi,ui,li].join(`
`);function ut(i,e){if(!i||!e)return;let{tokens:t}=e,r=i;ue.forEach(a=>{(!Object.prototype.hasOwnProperty.call(t,a.key)||t[a.key]===null||t[a.key]===void 0||t[a.key]==="")&&r.style.removeProperty(a.key)}),Object.entries(t).forEach(([a,n])=>{n!=null&&n!==""&&r.style.setProperty(a,n)})}var pi=`
  * {
    box-sizing: border-box;
    margin: 0;
    padding: 0;
