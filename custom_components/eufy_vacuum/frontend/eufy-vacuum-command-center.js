...
config/eufy_vacuum/battery/${this.escapeHtml(w)}/samples.jsonl</pre>
    ...
    `:""},i.renderRoomsActionBar=function(e,t,r,a,n){let c=r===1?"1 room":`${r} rooms`,s=(Array.isArray(a)?a:[]).filter(L=>L.enabled),o=e?n?"evcc-chip--start-warn":"evcc-chip--start":"disabled",l=this.card?._state,d=!!l?.hasActiveRun?.(),u=Number(this.card?._learningController?.getJobProgressPercent?.()??0),m=Array.isArray(l?.learningRoomTimeline?.())?l.learningRoomTimeline():[],v=l?.learningCompletedRooms?.()||[],p=new Set(v.map(L=>String(L.room_id))),f=s.reduce((L,O)=>{let G=String(O.id),F=m.find(re=>String(re.room_id)===G),Z=this.card?._state?.roomEstimateForRoom?.(O.id)??null,ce=Number(F?.minutes??Z?.minutes);return Number.isFinite(ce)?L+ce:L},0),h=Number(l?.dashboardPlannedJobEstimateTotalMinutes?.()),_=Number.isFinite(h)&&h>0?h:f,w=_>0?this._formatLearningDuration(_):null,x=l?.startConfirmation?.()??null,g=l?.startPreflight?.()??x?.preflight??null,R=!!l?.startRequiresConfirmation?.(),E=!!l?.cancelRunRequiresConfirmation?.(),C=!!l?.hasActiveRun?.(),P=!!l?.canPauseRun?.(),J=!!l?.canResumeRun?.(),oe=E?"Confirm Cancel":C?"Cancel Run":R?"Confirm Start":"Start Cleaning",he=E?"evcc-chip--start-warn evcc-chip--confirm-flash":C?"evcc-chip--cancel-run":R?"evcc-chip--start-warn evcc-chip--confirm-flash":o,z=R||E,me=Array.isArray(g?.blocked_rooms)?g.blocked_rooms:[],te=Array.isArray(g?.modified_rooms)?g.modified_rooms:[],le=Array.isArray(g?.warnings)?g.warnings:[];return`
    <div class="evcc-rooms-action-bar">
      ...
      ${s.length>0?`
        <div class="evcc-queue-chips">
          ...
          ${s.map((L,O)=>{let G=String(L.id),F=m.find($=>String($.room_id)===G),Z=p.has(G),ce=this.card?._learningController?.getRoomProgressSnapshot?.(L.id)??null,re="evcc-queue-chip--queued";d&&(F?.completed||Z||ce?.isCompleted?re="evcc-queue-chip--completed":F?.current||ce?.isCurrent?re="evcc-queue-chip--current":(F?.remaining||F)&&(re="evcc-queue-chip--remaining"));let ve="";if(F?.confidence_breakpoint?.ui_variant){let $=F.confidence_breakpoint.ui_variant;$==="success"?ve="evcc-queue-chip--confidence-high":$==="warning"?ve="evcc-queue-chip--confidence-medium":$==="error"&&(ve="evcc-queue-chip--confidence-low")}let ke=F?.minutes!=null?this._formatLearningMinutes(F.minutes):null,S=re==="evcc-queue-chip--completed"?100:re==="evcc-queue-chip--current"?Number(ce?.percent??u):0,D=re==="evcc-queue-chip--current"?`${Math.max(0,Math.min(99,Math.floor(S)))}%`:ke;return`
              <button
                type="button"
              ...
              >
                <span class="evcc-queue-chip-order">${O+1}</span>
  ...
  `},i.renderRoomCard=function(e,t){let r=this._normalizeRoomDisplayData(e),a=r.cleanMode?r.cleanModeLabel||this._formatCleanMode(r.cleanMode):null,c=!r.fanSpeed||["off","normal"].includes(String(r.fanSpeed).toLowerCase())?null:r.fanSpeedLabel||this._formatFanSpeed(r.fanSpeed),o=!r.cleanIntensity||String(r.cleanIntensity).toLowerCase()==="standard"?null:r.cleanIntensityLabel||this._formatCleanIntensity(r.cleanIntensity),l=this._isMopMode(r.cleanMode)&&r.waterLevel&&String(r.waterLevel).toLowerCase()!=="off"?r.waterLevelLabel||this._formatWaterLevel(r.waterLevel):null,d=this._isMopMode(r.cleanMode)&&r.edgeMopping?"Edge Mop On":null,u=Number(r.cleanPasses)>1?`${Number(r.cleanPasses)}\xD7 passes`:null,m=this.card?._state?.orderDragItemId?.(),v=this.card?._state?.orderDragOverItemId?.(),p=String(m)===String(r.id)?"evcc-order-drag-source":"",f=String(v)===String(r.id)?"evcc-order-drag-target":"",h=t?.roomEstimateForRoom?.(r.id)??null,_=t?.dashboardPlannedWaterRoomForRoom?.(r.id,r.slug)??null,w="";if(h&&h.error==null){let O=h.source==="learned"?"evcc-room-status--estimate-learned":"evcc-room-status--estimate-default",G=h.source==="learned"?this._formatLearningMinutes(h.minutes):`~${this._formatLearningMinutes(h.minutes)}`,F=[`Estimate: ${this._formatLearningMinutes(h.minutes)}`];h.source&&F.push(`Source: ${String(h.source)}`);let Z=Number(h.battery);Number.isFinite(Z)&&F.push(`Battery: ${Z}`);let ce=F.join(" \xB7 ");w=`
    ...
    `}let x="",g="",R="";if(h&&h.error==null&&typeof this.renderConfidenceChip=="function")if(h.source==="learned"){let O=h?.confidence_breakpoint?.ui_variant,G=O==="success"?"Reliable":O==="warning"?"Learning":O==="error"?"Uncertain":null;G&&(x=this.renderConfidenceChip(h.confidence_breakpoint,G,G),O==="success"?g="evcc-room-card--confidence-high":O==="warning"?g="evcc-room-card--confidence-medium":g="evcc-room-card--confidence-low")}else h.source==="default"&&(x=this.renderConfidenceChip({ui_variant:"neutral"},"Unlearned","Unlearned"));let E=String(_?.effective_clean_mode??_?.clean_mode??"").toLowerCase(),C=String(_?.effective_water_level??_?.water_level??"").toLowerCase();if(!!(_?.mop_active||this._isMopMode(E))&&C!=="off"){let O=Number(_.estimated_robot_water_used_ml);Number.isFinite(O)&&(R=`
        <div
      ...
      `)}let J=[];h?.intensity_mismatch&&J.push({text:"\u26A0 intensity mismatch",variant:"warning"});let oe=t?.troubleRoomForRoom?.(r.id)??null;if(oe?.is_trouble){let O=Number(oe.miss_count??0),G=Number(oe.run_count??0),F=Number(oe.miss_rate??0),Z=Number.isFinite(F)?Math.round(F*100):null;J.push({text:`\u26A0 Missed ${O}\xD7 of ${G} run${G===1?"":"s"}${Z!==null?` (${Z}%)`:""}`,variant:"warning",title:`This room was missed in ${Z??"?"}% of recent runs. Consider checking for obstacles or map accuracy.`})}let he=!!this.card?._state?.hasActiveRun?.(),z=this.card?._learningController?.getRoomProgressSnapshot?.(r.id)??null,me=z?.percent??this.card?._learningController?.getRoomProgressPercent?.(r.id),te=Number.isFinite(me)?me:0,le="evcc-room-card--queue-idle";r.enabled&&he&&(z?.isCompleted||te>=100?le="evcc-room-card--queue-completed":z?.isCurrent||te>0?le="evcc-room-card--queue-current":le="evcc-room-card--queue-remaining");let L=he&&z&&z.isCurrent?`
        <div class="evcc-room-progress-meta">
          <div
            class="evcc-room-status evcc-room-progress-chip"
            title="${this.escapeHtml([`Progress: ${z.percent}%`,Number.isFinite(z.elapsedMinutes)?`Elapsed: ${this._formatLearningMinutes(z.elapsedMinutes)}`:"",Number.isFinite(z.remainingMinutes)?`Remaining: ${this._formatLearningMinutes(z.remainingMinutes)}`:""].filter(Boolean).join(" \xB7 "))}"
          >
            ${this.escapeHtml(`${z.percent}% complete`)}
          </div>

          ${Number.isFinite(z.remainingMinutes)?`
            <div
              class="evcc-room-status evcc-room-progress-chip evcc-room-progress-chip--remaining"
              title="${this.escapeHtml([`Progress: ${z.percent}%`,Number.isFinite(z.elapsedMinutes)?`Elapsed: ${this._formatLearningMinutes(z.elapsedMinutes)}`:"",`Remaining: ${this._formatLearningMinutes(z.remainingMinutes)}`].filter(Boolean).join(" \xB7 "))}"
            >
              ${this.escapeHtml(`~${this._formatLearningMinutes(z.remainingMinutes)} left`)}
            </div>
          `:""}
        </div>
      `:"";return`
        ...
        <div class="evcc-room-notes">
          ${J.map(O=>`
            <div
              ...
              ${(()=>{let G=String(O.text).includes("No learned data")?"This room is using a fallback estimate until enough learned samples are collected.":String(O.text).includes("runs to reliable")?`Estimated ${String(O.text).split(" ")[0]} more runs to reach high confidence.`:String(O.text).includes("intensity mismatch")?"Estimate was learned from a different cleaning intensity or profile.":"",F=O.title||G;return F?`title="${this.escapeHtml(F)}"`:""})()}
            >
  ...
  `},i._normalizeRoomDisplayData=function(e){let t=e?.selected_profile_details??{},r=String(e?.profile_name??e?.profileName??e?.profile??"vacuum_quick"),a=String(e?.clean_mode??e?.cleanMode??t?.clean_mode??"vacuum"),n=String(e?.fan_speed??e?.fanSpeed??t?.fan_speed??""),c=String(e?.water_level??e?.waterLevel??t?.water_level??""),s=String(e?.clean_intensity??e?.cleanIntensity??t?.clean_intensity??""),o=Number(e?.clean_passes??e?.cleanPasses??e?.passes??t?.default_clean_passes??1),l=!!(e?.edge_mopping??e?.edgeMopping??t?.default_edge_mopping??!1),d=String(e?.floor_type??e?.floorType??""),u=String(e?.carpet_type??e?.carpetType??""),m=!!(e?.carpet??(()=>{let p=String(d).toLowerCase();return p==="carpet"||p.startsWith("carpet_")||p.startsWith("carpet-")})()),v=Number(e?.order??e?.displayOrder??e?.position??999999);return{id:e?.id,mapId:e?.mapId??e?.map_id??"",name:e?.name??e?.room_name??"",slug:e?.slug??e?.room_slug??null,enabled:!!e?.enabled,order:Number.isFinite(v)?v:999999,profileName:r,profileLabel:e?.profile_label??e?.profileLabel??e?.selected_profile_label??e?.resolved_profile_label??null,profileSubtitle:e?.profile_subtitle??e?.profileSubtitle??null,isCustomProfile:r.toLowerCase()==="custom",cleanMode:a,cleanModeLabel:e?.clean_mode_label??e?.cleanModeLabel??t?.clean_mode_label??null,fanSpeed:n,fanSpeedLabel:e?.fan_speed_label??e?.fanSpeedLabel??t?.fan_speed_label??null,waterLevel:c,waterLevelLabel:e?.water_level_label??e?.waterLevelLabel??t?.water_level_label??null,cleanIntensity:s,cleanIntensityLabel:e?.clean_intensity_label??e?.cleanIntensityLabel??t?.clean_intensity_label??t?.path_type_label??null,cleanPasses:Number.isFinite(o)?o:1,cleanPassesLabel:e?.clean_passes_label??e?.cleanPassesLabel??t?.clean_passes_label??null,edgeMopping:l,edgeMoppingLabel:e?.edge_mopping_label??e?.edgeMoppingLabel??t?.edge_mopping_label??null,floorType:d,floorTypeLabel:e?.floor_type_label??e?.floorTypeLabel??null,carpetType:u,carpetTypeLabel:e?.carpet_type_label??e?.carpetTypeLabel??null,carpet:m,selectedProfileDetails:t}},i._isMopMode=function(e){let t=String(e??"").toLowerCase();return t==="mop"||t==="vacuum_mop"||t.includes("mop")||t.includes("wash")},i._roomProfileLabel=function(e){let t=String(e??"").trim();return t?t.toLowerCase()==="custom"?"Custom":t==="vacuum_quick"?"Vacuum Only Quick":t==="vacuum_deep"?"Vacuum Only Deep":t==="vacuum_mop_quick"?"Quick":t==="vacuum_mop_deep"?"Deep":t==="user_1"?"User Profile 1":t.replace(/[_-]+/g," ").replace(/\b\w/g,r=>r.toUpperCase()):"Standard"},i._formatCleanMode=function(e){let t=String(e??"").trim().toLowerCase();return t==="vacuum_mop"||t==="vacuum and mop"?"Vacuum + Mop":t==="vacuum"?"Vacuum":t==="mop"?"Mop":this._formatSettingValue(e)},i._formatFanSpeed=function(e){return this._formatSettingValue(e)},i._formatWaterLevel=function(e){return this._formatSettingValue(e)},i._formatCleanIntensity=function(e){return this._formatSettingValue(e)},i._formatFloorType=function(e){return this._formatSettingValue(e)},i._formatSettingValue=function(e){return e?String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}}function Cr(i){i.renderRunProfilesPanel=function(e){let t=e.savedRunProfiles?.()??[],r=e.selectedRunProfile?.()??null,a=e.runProfileDraft?.()??{name:"",expose_as_button:!1},n=!!e.isRunProfileEditorOpen?.(),c=e.runProfileEditorMode?.()??"new";return`
    ...
    `}}function Dr(i){if(!i)return null;let e=String(i).trim();if(!/^color-mix\(/i.test(e))return null;let t=e.indexOf("("),r=e.lastIndexOf(")");if(t===-1||r===-1)return null;let c=e.slice(t+1,r).replace(/^\s*in\s+\w+\s*,\s*/i,"").match(/^(.*?\s+\d+(?:\.\d+)?%)\s*,\s*(.*?\s+\d+(?:\.\d+)?%)\s*$/);if(!c)return null;let s=/^(.*?)\s+(\d+(?:\.\d+)?)%$/,o=c[1].trim().match(s),l=c[2].trim().match(s);return!o||!l?null:{color1:o[1].trim(),ratio:parseFloat(o[2]),color2:l[1].trim(),ratio2:parseFloat(l[2])}}function Ei(i,e,t){let r=Math.max(0,Math.min(100,Math.round(e)));return`color-mix(in srgb, ${i} ${r}%, ${t} ${100-r}%)`}var Ne=new Set(["--evcc-accent","--evcc-surface-base","--evcc-text-primary","--evcc-radius-card"]),ki={"Shared Foundations":{min:0,max:64,step:2},"Cards & Surfaces":{min:0,max:32,step:1},"Borders & Shadows":{min:0,max:32,step:1},Chips:{min:20,max:48,step:1},"Room Cards":{min:0,max:32,step:1},"Floor Textures":{min:0,max:1,step:.01},"Floor Textures \u2014 Tile":{min:0,max:1,step:.01},"Floor Textures \u2014 Wood":{min:0,max:1,step:.01},"Floor Textures \u2014 Marble":{min:0,max:1,step:.01},"Floor Textures \u2014 Concrete":{min:0,max:1,step:.01},"Floor Textures \u2014 Carpet Low":{min:0,max:1,step:.01},"Floor Textures \u2014 Carpet High":{min:0,max:1,step:.01},"Floor Textures \u2014 Granite":{min:0,max:1,step:.01},"Queue & Ordering":{min:0,max:32,step:1},"Status, Confidence & Alerts":{min:0,max:32,step:1},"Learning & Metrics":{min:0,max:32,step:1},"Modals & Overlays":{min:0,max:32,step:1}};function Ti(i){let e=parseFloat(String(i||"").trim());return Number.isNaN(e)?null:e}function Br(i,e){let t=String(e||"").trim();if(!t)return{numeric:null,unit:Fe(i)};if(i.type==="number")return{numeric:Ti(t),unit:""};if(i.type==="size"){let r=t.match(/^(-?\d*\.?\d+)\s*(px|rem|em|%|vh|vw|vmin|vmax|ch|ex)$/i);return r?{numeric:Number(r[1]),unit:r[2].toLowerCase()}:{numeric:null,unit:Fe(i)}}if(i.type==="duration"){let r=t.match(/^(-?\d*\.?\d+)\s*(ms|s)$/i);return r?{numeric:Number(r[1]),unit:r[2].toLowerCase()}:{numeric:null,unit:Fe(i)}}return{numeric:null,unit:""}}function Fe(i){return i.type==="size"?"px":i.type==="duration"?"ms":""}function $i(i){return i.type==="size"||i.type==="number"||i.type==="duration"}function Mi(i,e){return $i(i)?e==null||e===""?!0:Br(i,e).numeric!==null:!1}function Ai(i){let e=Number(i);return Number.isNaN(e)?100:Math.max(0,Math.min(100,e))}function Ii(i){let e=String(i||"").trim();if(/^#[0-9a-fA-F]{8}$/.test(e)){let t=e.slice(7,9),r=parseInt(t,16)/255;return Ai(Math.round(r*100))}return 100}function jr(i){i.renderThemeView=function(){let e=this.card._state._ensureThemeState(),{tokens:t,sources:r}=this.card._state.resolvedTheme(),a=e.activeSubTab||"presets";return`
    ...
    `},i._renderThemeTokenRow=function(e,t,r){let a=r==="draft",n=t||"";return e.type==="color"?Dr(n)?this._renderThemeColorMixTokenRow(e,n,a):this._renderThemeColorTokenRow(e,n,a):Mi(e,n)?this._renderThemeNumericTokenRow(e,n,a):this._renderThemeTextTokenRow(e,n,a)},i._renderThemeColorTokenRow=function(e,t,r){let a=String(t||"").trim(),n=this._safeColorInputValue(a),c=Ii(a),s=/^#[0-9a-fA-F]{8}$/.test(a)?`#${a.slice(1,7)}`:a;return`
    ...
    `}}function Ci(i){switch(i){case"cleaning":return"alert";case"returning":return"walking";case"paused":return"standing";case"error":return"warning";case"docked":case"idle":return"curled";default:return"curled"}}function qr(i){let e=0,t=0,r=0,a=i.length;for(let n=0,c=a-1;n<a;c=n++){let s=i[c][0]*i[n][1]-i[n][0]*i[c][1];e+=s,t+=(i[c][0]+i[n][0])*s,r+=(i[c][1]+i[n][1])*s}if(e*=.5,Math.abs(e)<1e-10){let n=i.reduce((s,o)=>s+o[0],0),c=i.reduce((s,o)=>s+o[1],0);return[n/a,c/a]}return[t/(6*e),r/(6*e)]}var He=["#00e5ff","#ff6b35","#a3e635","#e879f9","#fbbf24","#a78bfa","#fb7185","#34d399","#60a5fa","#f472b6","#4ade80","#f97316"],Li=[{key:"dark",label:"Dark",hint:"primary \u2014 clearest room colours"},{key:"light",label:"Light",hint:"assist \u2014 wall detection"},{key:"default",label:"Default",hint:"fallback"}];function Gr(i){i.renderMapRoomView=function(e){let{state:t,vacuumStatus:r}=e,a=t.mapSegmentsData(),n=t.mapImageUrl();if(!a?.available||!n)return`
    ...
    />`,u="";return c&&(u=n.map(([m,v],p)=>{let f=a===p;return`<circle
          ...
          data-segment-id="${l}"
          data-vertex-index="${p}"
        />`}).join("")),`<g>${d}${u}</g>`},i._renderVariantsSection=function(e,t,r){let a=Li.map(({key:l,label:d,hint:u})=>{let m=e[l],v=r?.type==="upload"&&r?.variant===l&&r?.status==="busy",p=r?.type==="upload"&&r?.variant===l&&r?.status==="error",f=m?`${m.width} \xD7 ${m.height}`:"not uploaded",h=m?"evcc-map-variant-status--ok":"evcc-map-variant-status--missing";return`
    ...
    `}}var N="/eufy_vacuum/textures",De={tile:{opacityDefault:1,layers:[{url:`${N}/tile/tile-mask.png`,role:"base",colorToken:"--evcc-floor-tile-base",colorDefault:"#D4AF37",opacityToken:"--evcc-floor-tile-face-opacity",opacityDefault:.87},{url:`${N}/tile/grout-mask.png`,role:"grout",colorToken:"--evcc-floor-tile-grout",colorDefault:"#121212",opacityToken:"--evcc-floor-tile-grout-opacity",opacityDefault:.95},{url:`${N}/tile/pure-tile-grout.png`,role:"accent",colorToken:"--evcc-floor-tile-accent",colorDefault:"#f0f0f5",opacityToken:"--evcc-floor-tile-line-opacity",opacityDefault:.39}],masks:[{url:`${N}/tile/tile-mask.png`},{url:`${N}/tile/grout-mask.png`},{url:`${N}/tile/pure-tile-grout.png`}],baseTexture:null},wood:{opacityDefault:.99,layers:[{url:`${N}/wood/wood-directional-depth-mask.png`,role:"base",colorToken:"--evcc-floor-wood-base",colorDefault:"#7A4010cf",opacityToken:"--evcc-floor-wood-depth-opacity",opacityDefault:.43},{url:`${N}/wood/wood-grain-mask.png`,role:"base",colorToken:"--evcc-floor-wood-base",colorDefault:"#7A4010cf",opacityToken:"--evcc-floor-wood-grain-opacity",opacityDefault:.84},{url:`${N}/wood/wood-seam-mask.png`,role:"accent",colorToken:"--evcc-floor-wood-accent",colorDefault:"#e89754",opacityToken:"--evcc-floor-wood-seam-opacity",opacityDefault:.78}],masks:[{url:`${N}/wood/wood-grain-mask.png`},{url:`${N}/wood/wood-seam-mask.png`},{url:`${N}/wood/wood-directional-depth-mask.png`}],baseTexture:null},marble:{opacityDefault:.9,layers:[{url:`${N}/marble/marble-base-mask.png`,role:"base",colorToken:"--evcc-floor-marble-base",colorDefault:"#e9e8e8",opacityToken:"--evcc-floor-marble-base-opacity",opacityDefault:.97},{url:`${N}/marble/marble-micro-texture-mask.png`,role:"micro",colorToken:"--evcc-floor-marble-micro",colorDefault:"#080707",opacityToken:"--evcc-floor-marble-micro-opacity",opacityDefault:1},{url:`${N}/marble/marble-vein-mask.png`,role:"accent",colorToken:"--evcc-floor-marble-accent",colorDefault:"#D4AF3773",opacityToken:"--evcc-floor-marble-vein-opacity",opacityDefault:0}],masks:[{url:`${N}/marble/marble-vein-mask.png`},{url:`${N}/marble/marble-micro-texture-mask.png`},{url:`${N}/marble/marble-base-mask.png`}],baseTexture:null},concrete:{opacityDefault:1,layers:[{url:`${N}/concrete/concrete-broad-mask.png`,role:"base",colorToken:"--evcc-floor-concrete-base",colorDefault:"#eceaea",opacityToken:"--evcc-floor-concrete-broad-opacity",opacityDefault:1},{url:`${N}/concrete/concrete-micro-mask.png`,role:"accent",colorToken:"--evcc-floor-concrete-accent",colorDefault:"#121111",opacityToken:"--evcc-floor-concrete-micro-opacity",opacityDefault:.62}],masks:[{url:`${N}/concrete/concrete-micro-mask.png`},{url:`${N}/concrete/concrete-broad-mask.png`}],baseTexture:null},carpet_low:{opacityDefault:.8,layers:[{url:`${N}/carpet/texture-floor-carpet-low.png`,role:"base",colorToken:"--evcc-floor-carpet-low-base",colorDefault:"#0d0c0c",opacityToken:"--evcc-floor-carpet-low-texture-opacity",opacityDefault:1}],masks:[],baseTexture:`${N}/carpet/texture-floor-carpet-low.png`},carpet_high:{opacityDefault:1,layers:[{url:`${N}/carpet/texture-floor-carpet-high.png`,role:"base",colorToken:"--evcc-floor-carpet-high-base",colorDefault:"#0a0a0a",opacityToken:"--evcc-floor-carpet-high-texture-opacity",opacityDefault:1}],masks:[],baseTexture:`${N}/carpet/texture-floor-carpet-high.png`},granite_light:{opacityDefault:1,layers:[{url:`${N}/granite/texture-floor-granite-light.png`,role:"base",colorToken:"--evcc-floor-granite-light-base",colorDefault:"#0a0a0a",opacityToken:"--evcc-floor-granite-light-texture-opacity",opacityDefault:1}],masks:[],baseTexture:`${N}/granite/texture-floor-granite-light.png`},default:{opacityDefault:.85,layers:[],masks:[],baseTexture:null}};function We(i){let e=De[i];return e?e.baseTexture?e.baseTexture:e.layers.length?e.layers[0].url:e.masks.length?e.masks[0].url:null:null}function Je(i){let e=String(i?.floor_type??"").toLowerCase().trim(),t=String(i?.carpet_type??"").toLowerCase().trim();return e==="carpet"?t==="high_pile"||t==="high"?"carpet_high":"carpet_low":e==="carpet_low_pile"||e==="carpet_low"?"carpet_low":e==="carpet_high_pile"||e==="carpet_high"?"carpet_high":e==="hardwood"||e==="laminate"||e==="wood"?"wood":e==="tile"?"tile":e==="marble"?"marble":e==="concrete"?"concrete":e==="granite"||e==="granite_light"?"granite_light":"default"}function Oi(i){let e=String(i??""),t=2166136261;for(let n=0;n<e.length;n++)t^=e.charCodeAt(n),t=Math.imul(t,16777619)>>>0;let r=t%101,a=(t>>>13^t>>>7)%101;return`${r}% ${a}%`}function Ur(i){i._resolveSegmentFloorType=function(e){return Je({floor_type:e?.floor_type??e?.floorType??"",carpet_type:e?.carpet_type??e?.carpetType??""})},i._renderFloorTextureLayer=function(e){let t=Je({floor_type:e?.floorType??"",carpet_type:e?.carpetType??""}),r=De[t]??De.default,a=r.opacityDefault??.85,n=`var(--evcc-floor-${t}-opacity-card,var(--evcc-floor-texture-opacity-card,${a}))`,c=Oi(e?.id??e?.name??t),s=r.layers.map(o=>{let l=`var(${o.colorToken},${o.colorDefault})`,d=`url(${o.url})`,u=`var(${o.opacityToken},${o.opacityDefault})`;return`<span class="evcc-ftx-layer" data-role="${o.role}" style="background-color:${l};mask-image:${d};-webkit-mask-image:${d};--layer-opacity:${u}"></span>`}).join("");return`<div class="evcc-room-texture-layer" data-floor="${t}" style="--floor-opacity-card:${n};--floor-position-card:${c}">${s}</div>`},i._buildFloorTextureDefs=function(e){let t=new Set,r=[];for(let a of e){if(t.has(a))continue;t.add(a);let n=We(a);n&&r.push(`<pattern id="evcc-ftx-${a}" patternUnits="userSpaceOnUse" width="8" height="8"><image href="${n}" width="8" height="8" preserveAspectRatio="xMidYMid slice"/></pattern>`)}return r.length?`<defs>${r.join("")}</defs>`:""},i._renderFloorTexturePolygon=function(e,t){let r=e.polygon_pct;return!Array.isArray(r)||r.length<3||!We(t)?"":`<polygon class="evcc-map-texture-polygon" points="${r.map(([n,c])=>`${n},${c}`).join(" ")}" fill="url(#evcc-ftx-${t})" data-floor="${t}"/>`}}var Pi=[{value:"hardwood",label:"Hardwood"},{value:"laminate",label:"Laminate"},{value:"tile",label:"Tile"},{value:"marble",label:"Marble"},{value:"granite",label:"Granite"},{value:"concrete",label:"Concrete"},{value:"carpet_low_pile",label:"Low-Pile Carpet"},{value:"carpet_high_pile",label:"High-Pile Carpet"}];function Jr(i){i.renderSetupView=function(e){let{state:t,card:r}=e,a=r._config?.vacuum_entity_id??"",n=t.setupStatus?.()??null,c=t.setupLoading?.()??!1,s=t.setupError?.()??null,o=t.setupLastResult?.()??null,d=(Array.isArray(n?.vacuums)?n.vacuums:[]).find(S=>S.vacuum_entity_id===a)??null,u=Array.isArray(d?.setup_steps)&&d.setup_steps.length?d.setup_steps:Ni(d),m=d?.room_drift??null,v=t.setupRoomEditorOpenMapId?.()??null,p=t.setupRoomEditorLoadingMapId?.()??null,f=t.setupRoomEditorRooms?.()??[],h=t.setupRoomEditorSaving?.()??!1,_=t.setupDeletePendingMapId?.()??null,w=t.setupDeleteStage?.()??null,x=t.setupDeleteTypedToken?.()??"",g=t.setupDeleteDeleting?.()??!1,R=new Set((t.setupRoomEditorEnabledIds?.()??[]).map(String)),E=t.setupRoomEditorFloorTypesMap?.()??{},C=(d?.maps??[]).filter(S=>S.imported),P=c?'<div class="evcc-setup-result info">Working\u2026</div>':"",J=s&&!c?`<div class="evcc-setup-result error">${this.escapeHtml(String(s))}</div>`:"",oe=(()=>{if(!o||c)return"";let S=o.status??"",D=o.message??"";return S==="error"||S==="blocked"?`<div class="evcc-setup-result error">${this.escapeHtml(D)}</div>`:D?`<div class="evcc-setup-result success">${this.escapeHtml(D)}</div>`:""})(),te={add_vacuum:S=>S.completed?`
      ...
      `,import_active_map:S=>{let D=Wr(u,"add_vacuum"),$=C.length;if(!D)return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';let V=$>0?`<div class="evcc-setup-step-body muted">${$} map${$===1?"":"s"} imported.</div>`:`<div class="evcc-setup-step-body">Import the vacuum's currently active map. Make sure it has completed a mapping run first.</div>`,W=$>0?"Import Another Map":"Import Active Map",Y=$>0?"secondary":"";return`
        ${V}
      ...
      `},save_rooms:S=>{let D=u.find(H=>H.id==="import_active_map"),$=!!D,V=!D||D.completed;if(!Wr(u,"add_vacuum"))return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';if($&&!V)return'<div class="evcc-setup-step-body muted">Complete map import first.</div>';if(C.length===0&&!$)return`
          <div class="evcc-setup-step-body">
      ...
      `}},le=(S,D)=>{if(!S||S.in_sync)return"";let $=Array.isArray(S.new_rooms)?S.new_rooms:[],V=Array.isArray(S.removed_rooms)?S.removed_rooms:[],W=Array.isArray(S.transiently_missing)?S.transiently_missing:[];if($.length===0&&V.length===0&&W.length===0)return"";let Y=$.length===0?"":`
      ...
      `},L=S=>p===S?`<div class="evcc-setup-room-editor">
        ...
        <div class="evcc-setup-room-editor">
          ...
          <div class="evcc-setup-room-list">
            ${f.length===0?'<div class="evcc-setup-step-body muted">No rooms found for this map.</div>':f.map($=>{let V=String($.room_id),W=this.escapeHtml($.name??`Room ${V}`),Y=R.has(V),ue=E[V]??"hardwood",fe=Pi.map(H=>`
      ...
      `,O=(S,D)=>{if(_!==S)return"";let $=this.escapeHtml(D?.typed_confirmation_value??`Map ${S}`),V=D?.requires_typed_confirmation??!1,W=D?.reasons??[],Y=W.length?`<div class="evcc-setup-delete-badges">
      ...
      `},G=(S,D)=>{let $=String(S.map_id),V=this.escapeHtml(S.display_name??`Map ${$}`),W=t.isSetupMapConfigured?.($),Y=v===$||p===$,ue=S.protection??null,fe=ue?.requires_typed_confirmation??!1,H=_===$,si=W&&!Y?'<span class="evcc-setup-configured-badge">\u2713 Configured</span>':"",oi=D?`
      ...
      `},F=(S,D)=>{let $=te[S.id],V=$?$(S):`<div class="evcc-setup-step-body muted">No handler for step "${this.escapeHtml(S.id)}".</div>`,W=S.completed?"\u2713":String(D+1);return`
...
`;var Na=[fa,ga,ba,_a,ya,xa,wa,Sa,Ye,Qe,Ra,Ea,ka,Ta,$a,Ma,Aa,Ia,Ca,La,Oa,Pa].join(`
