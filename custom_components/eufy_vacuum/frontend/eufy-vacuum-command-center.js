...
config/eufy_vacuum/battery/${this.escapeHtml(w)}/samples.jsonl</pre>
    ...
    `:""},i.renderRoomsActionBar=function(e,t,r,a,c){let n=r===1?"1 room":`${r} rooms`,s=(Array.isArray(a)?a:[]).filter(k=>k.enabled),o=e?c?"evcc-chip--start-warn":"evcc-chip--start":"disabled",l=this.card?._state,d=!!l?.hasActiveRun?.(),u=Number(this.card?._learningController?.getJobProgressPercent?.()??0),m=Array.isArray(l?.learningRoomTimeline?.())?l.learningRoomTimeline():[],p=l?.learningCompletedRooms?.()||[],v=new Set(p.map(k=>String(k.room_id))),f=s.reduce((k,G)=>{let Y=String(G.id),J=m.find(ae=>String(ae.room_id)===Y),Te=this.card?._state?.roomEstimateForRoom?.(G.id)??null,de=Number(J?.minutes??Te?.minutes);return Number.isFinite(de)?k+de:k},0),h=Number(l?.dashboardPlannedJobEstimateTotalMinutes?.()),b=Number.isFinite(h)&&h>0?h:f,w=b>0?this._formatLearningDuration(b):null,S=l?.startConfirmation?.()??null,g=l?.startPreflight?.()??S?.preflight??null,R=!!l?.startRequiresConfirmation?.(),y=!!l?.cancelRunRequiresConfirmation?.(),T=!!l?.clearQueueRequiresConfirmation?.(),M=!!l?.hasActiveRun?.(),W=!!l?.canPauseRun?.(),B=!!l?.canResumeRun?.(),he=y?"Confirm Cancel":M?"Cancel Run":R?"Confirm Start":"Start Cleaning",j=y?"evcc-chip--start-warn evcc-chip--confirm-flash":M?"evcc-chip--cancel-run":R?"evcc-chip--start-warn evcc-chip--confirm-flash":o,Re=R||y,ne=Array.isArray(g?.blocked_rooms)?g.blocked_rooms:[],ue=Array.isArray(g?.modified_rooms)?g.modified_rooms:[],fe=Array.isArray(g?.warnings)?g.warnings:[],ge=l?.startMopCarpetWarning?.()??null,be=l?.startOrderAdvisory?.()??null,I=l?.strictOrder?.()??!1;return`
    <div class="evcc-rooms-action-bar">
        ...
        <div class="evcc-chips">
          ...
          ${M&&(W||B)?`
            <button
              type="button"
              class="evcc-chip"
              data-action="${B?"resume-run":"pause-run"}"
            >${B?"Resume":"Pause"}</button>
          `:""}
        ...
        <div class="evcc-start-preflight-panel">
          ...
          ${ue.length?`
            <div class="evcc-start-preflight-section">
              ...
              <div class="evcc-start-preflight-list">
                ${ue.map(k=>{let G=Object.keys(k.changes??{}).join(", ")||"Settings adjusted",Y=k.derived&&k.source_rule_name?` (via ${k.source_room_name??"another room"}'s ${k.source_rule_name})`:"";return`
                    <div class="evcc-start-preflight-item">
                      <span class="evcc-start-preflight-room">${this.escapeHtml(k.name??k.room_id??"Room")}</span>
      ...
      ${s.length>0?`
        <div class="evcc-queue-chips">
          ...
          ${s.map((k,G)=>{let Y=String(k.id),J=m.find(E=>String(E.room_id)===Y),Te=v.has(Y),de=this.card?._learningController?.getRoomProgressSnapshot?.(k.id)??null,ae="evcc-queue-chip--queued";d&&(J?.completed||Te||de?.isCompleted?ae="evcc-queue-chip--completed":J?.current||de?.isCurrent?ae="evcc-queue-chip--current":J?.skipped||de?.isSkipped?ae="evcc-queue-chip--skipped":(J?.remaining||J)&&(ae="evcc-queue-chip--remaining"));let qe=J?.running_long||de?.isRunningLong?"evcc-queue-chip--running-long":"",_e="";if(J?.confidence_breakpoint?.ui_variant){let E=J.confidence_breakpoint.ui_variant;E==="success"?_e="evcc-queue-chip--confidence-high":E==="warning"?_e="evcc-queue-chip--confidence-medium":E==="error"&&(_e="evcc-queue-chip--confidence-low")}let $e=J?.minutes!=null?this._formatLearningMinutes(J.minutes):null,Ge=ae==="evcc-queue-chip--completed"?100:ae==="evcc-queue-chip--current"?Number(de?.percent??u):0,Ue=ae==="evcc-queue-chip--current"?`${Math.max(0,Math.min(99,Math.floor(Ge)))}%`:$e;return`
              <button
                type="button"
  ...
  `},i.renderRoomCard=function(e,t){let r=this._normalizeRoomDisplayData(e),a=r.cleanMode?r.cleanModeLabel||this._formatCleanMode(r.cleanMode):null,n=!r.fanSpeed||["off","normal"].includes(String(r.fanSpeed).toLowerCase())?null:r.fanSpeedLabel||this._formatFanSpeed(r.fanSpeed),o=!r.cleanIntensity||String(r.cleanIntensity).toLowerCase()==="standard"?null:r.cleanIntensityLabel||this._formatCleanIntensity(r.cleanIntensity),l=this._isMopMode(r.cleanMode)&&r.waterLevel&&String(r.waterLevel).toLowerCase()!=="off"?r.waterLevelLabel||this._formatWaterLevel(r.waterLevel):null,d=this._isMopMode(r.cleanMode)&&r.edgeMopping?"Edge Mop On":null,u=Number(r.cleanPasses)>1?`${Number(r.cleanPasses)}\xD7 passes`:null,m=this.card?._state?.orderDragItemId?.(),p=this.card?._state?.orderDragOverItemId?.(),v=String(m)===String(r.id)?"evcc-order-drag-source":"",f=String(p)===String(r.id)?"evcc-order-drag-target":"",h=t?.roomEstimateForRoom?.(r.id)??null,b=t?.dashboardPlannedWaterRoomForRoom?.(r.id,r.slug)??null,w="";if(h&&h.error==null){let I=h.source==="learned"?"evcc-room-status--estimate-learned":"evcc-room-status--estimate-default",k=h.source==="learned"?this._formatLearningMinutes(h.minutes):`~${this._formatLearningMinutes(h.minutes)}`,G=[`Estimate: ${this._formatLearningMinutes(h.minutes)}`];h.source&&G.push(`Source: ${String(h.source)}`);let Y=Number(h.battery);Number.isFinite(Y)&&G.push(`Battery: ${Y}`);let J=G.join(" \xB7 ");w=`
    ...
    `}let S="",g="",R="";if(h&&h.error==null&&typeof this.renderConfidenceChip=="function")if(h.source==="learned"){let I=h?.confidence_breakpoint?.ui_variant,k=I==="success"?"Reliable":I==="warning"?"Learning":I==="error"?"Uncertain":null;k&&(S=this.renderConfidenceChip(h.confidence_breakpoint,k,k),I==="success"?g="evcc-room-card--confidence-high":I==="warning"?g="evcc-room-card--confidence-medium":g="evcc-room-card--confidence-low")}else h.source==="default"&&(S=this.renderConfidenceChip({ui_variant:"neutral"},"Unlearned","Unlearned"));let y=String(b?.effective_clean_mode??b?.clean_mode??"").toLowerCase(),T=String(b?.effective_water_level??b?.water_level??"").toLowerCase();if(!!(b?.mop_active||this._isMopMode(y))&&T!=="off"){let I=Number(b.estimated_robot_water_used_ml);Number.isFinite(I)&&(R=`
        <div
      ...
      `)}let W=[];h?.intensity_mismatch&&W.push({text:"\u26A0 intensity mismatch",variant:"warning"});let B=t?.troubleRoomForRoom?.(r.id)??null;if(B?.is_trouble){let I=Number(B.miss_count??0),k=Number(B.run_count??0),G=Number(B.miss_rate??0),Y=Number.isFinite(G)?Math.round(G*100):null;W.push({text:`\u26A0 Missed ${I}\xD7 of ${k} run${k===1?"":"s"}${Y!==null?` (${Y}%)`:""}`,variant:"warning",title:`This room was missed in ${Y??"?"}% of recent runs. Consider checking for obstacles or map accuracy.`})}let he=!!this.card?._state?.hasActiveRun?.(),j=this.card?._learningController?.getRoomProgressSnapshot?.(r.id)??null,Re=j?.percent??this.card?._learningController?.getRoomProgressPercent?.(r.id),ne=Number.isFinite(Re)?Re:0,ue="evcc-room-card--queue-idle";r.enabled&&he&&(j?.isCompleted||ne>=100?ue="evcc-room-card--queue-completed":j?.isCurrent||ne>0?ue="evcc-room-card--queue-current":ue="evcc-room-card--queue-remaining");let fe="",ge=this.formatRelativeAgo?.(r.lastCleanedAt);if(ge&&!j?.isCurrent){let I=[`Last cleaned: ${r.lastCleanedAt}`];r.lastJobMode&&I.push(`Mode: ${String(r.lastJobMode)}`),fe=`
      <div
        class="evcc-room-status evcc-room-status--last-cleaned"
        title="${this.escapeHtml(I.join(" | "))}"
      >${this.escapeHtml(ge)}</div>
    `}let be=he&&j&&j.isCurrent?`
  ...
  `},i._normalizeRoomDisplayData=function(e){let t=e?.selected_profile_details??{},r=String(e?.profile_name??e?.profileName??e?.profile??"vacuum_quick"),a=String(e?.clean_mode??e?.cleanMode??t?.clean_mode??"vacuum"),c=String(e?.fan_speed??e?.fanSpeed??t?.fan_speed??""),n=String(e?.water_level??e?.waterLevel??t?.water_level??""),s=String(e?.clean_intensity??e?.cleanIntensity??t?.clean_intensity??""),o=Number(e?.clean_passes??e?.cleanPasses??e?.passes??t?.default_clean_passes??1),l=!!(e?.edge_mopping??e?.edgeMopping??t?.default_edge_mopping??!1),d=String(e?.floor_type??e?.floorType??""),u=String(e?.carpet_type??e?.carpetType??""),m=!!(e?.carpet??(()=>{let v=String(d).toLowerCase();return v==="carpet"||v.startsWith("carpet_")||v.startsWith("carpet-")})()),p=Number(e?.order??e?.displayOrder??e?.position??999999);return{id:e?.id,mapId:e?.mapId??e?.map_id??"",name:e?.name??e?.room_name??"",slug:e?.slug??e?.room_slug??null,enabled:!!e?.enabled,order:Number.isFinite(p)?p:999999,profileName:r,profileLabel:e?.profile_label??e?.profileLabel??e?.selected_profile_label??e?.resolved_profile_label??null,profileSubtitle:e?.profile_subtitle??e?.profileSubtitle??null,lastCleanedAt:e?.lastCleanedAt??e?.last_cleaned_at??null,lastJobMode:e?.lastJobMode??e?.last_job_mode??null,isCustomProfile:r.toLowerCase()==="custom",cleanMode:a,cleanModeLabel:e?.clean_mode_label??e?.cleanModeLabel??t?.clean_mode_label??null,fanSpeed:c,fanSpeedLabel:e?.fan_speed_label??e?.fanSpeedLabel??t?.fan_speed_label??null,waterLevel:n,waterLevelLabel:e?.water_level_label??e?.waterLevelLabel??t?.water_level_label??null,cleanIntensity:s,cleanIntensityLabel:e?.clean_intensity_label??e?.cleanIntensityLabel??t?.clean_intensity_label??t?.path_type_label??null,cleanPasses:Number.isFinite(o)?o:1,cleanPassesLabel:e?.clean_passes_label??e?.cleanPassesLabel??t?.clean_passes_label??null,edgeMopping:l,edgeMoppingLabel:e?.edge_mopping_label??e?.edgeMoppingLabel??t?.edge_mopping_label??null,floorType:d,floorTypeLabel:e?.floor_type_label??e?.floorTypeLabel??null,carpetType:u,carpetTypeLabel:e?.carpet_type_label??e?.carpetTypeLabel??null,carpet:m,selectedProfileDetails:t}},i._isMopMode=function(e){let t=String(e??"").toLowerCase();return t==="mop"||t==="vacuum_mop"||t.includes("mop")||t.includes("wash")},i._roomProfileLabel=function(e){let t=String(e??"").trim();return t?t.toLowerCase()==="custom"?"Custom":t==="vacuum_quick"?"Vacuum Only Quick":t==="vacuum_deep"?"Vacuum Only Deep":t==="vacuum_mop_quick"?"Quick":t==="vacuum_mop_deep"?"Deep":t==="user_1"?"User Profile 1":t.replace(/[_-]+/g," ").replace(/\b\w/g,r=>r.toUpperCase()):"Standard"},i._formatCleanMode=function(e){let t=String(e??"").trim().toLowerCase();return t==="vacuum_mop"||t==="vacuum and mop"?"Vacuum + Mop":t==="vacuum"?"Vacuum":t==="mop"?"Mop":this._formatSettingValue(e)},i._formatFanSpeed=function(e){return this._formatSettingValue(e)},i._formatWaterLevel=function(e){return this._formatSettingValue(e)},i._formatCleanIntensity=function(e){return this._formatSettingValue(e)},i._formatFloorType=function(e){return this._formatSettingValue(e)},i._formatSettingValue=function(e){return e?String(e).replace(/[_-]+/g," ").replace(/\b\w/g,t=>t.toUpperCase()):""}}function Fa(i){i.renderRunProfilesPanel=function(e){let t=e.savedRunProfiles?.()??[],r=e.selectedRunProfile?.()??null,a=e.runProfileDraft?.()??{name:"",expose_as_button:!1},c=!!e.isRunProfileEditorOpen?.(),n=e.runProfileEditorMode?.()??"new";return`
      ...
      <div class="evcc-modal-backdrop" data-action="close-maintenance-modal">
        <div class="evcc-modal evcc-maintenance-modal" data-stop-propagation>
          ...
          <div class="evcc-modal-body">
            ...
            ${c==="maintenance"?(()=>{let g=Number(r?.interval_hours),R=Number(r?.default_interval_hours),y=Number(r?.max_interval_hours),T=r?.reset_service_data?.vacuum_entity_id??"",M=r?.component??"",W=Number.isFinite(g)&&g>0?g:Number.isFinite(R)?R:"",B=[];return Number.isFinite(R)&&R>0&&B.push(`Default ${R}h`),Number.isFinite(y)&&y>0&&B.push(`Max ${y}h`),`
                <div class="evcc-editor-field-group">
                  ...
                  ${B.length?`
                    <div class="evcc-maintenance-interval-hint">${this.escapeHtml(B.join(" \xB7 "))}</div>
                  `:""}
                </div>
              `})():""}
    ...
    `},i._renderThemePalette=function(e,t){let r=me.filter(a=>et.has(a.key));return`
    ...
    `},i._renderThemeTokenEditor=function(e,t){let r=this.card._state.getThemeGroupFilter(),a={},c=new Set;for(let o of ce){let l=o.indexOf(" \u2014 ");if(l===-1)continue;let d=o.slice(0,l);ce.includes(d)&&((a[d]=a[d]??[]).push(o),c.add(o))}let n=(o,l=!1)=>{let d=this.card._state.filteredThemeTokensForGroup(o,me,{excludeKeys:et}),u=this.card._state.getThemeGroupSearchQuery(o),m=String(u||"").trim().length>0,p=r===o||m,f=(a[o]??[]).map(g=>n(g,!0)).filter(Boolean).join("");if(!d.length&&!p&&!f)return"";let h=this.card._state.themeGroupCounts(o,me,{excludeKeys:et}),w=this.card._state.shouldForceThemeGroupOpenForSearch(o,me,{excludeKeys:et})||this.card._state.isThemeGroupOpen(o),S=l?o.slice(o.lastIndexOf(" \u2014 ")+3):o;return`
        <div
      ...
      <div class="evcc-map-view">
        <div class="evcc-map-container">
          ...
          <div class="evcc-map-layers" style="transform:translate(${p}px,${v}px) scale(${m});transform-origin:0 0">
            ...
            ${s.map(f=>{let h=f.polygon_pct;if(!Array.isArray(h)||h.length<3)return"";let[b,w]=ei(h),S=Math.min(Math.max(b,5),95),g=Math.min(Math.max(w,6),94),R=r.roomIdForSegment(f.segment_id),T=(R!=null?d.find(B=>String(B.id)===String(R)):null)?.name??f.name??f.label??null;if(!T)return"";let M=l.findIndex(B=>String(B.segment_id)===String(f.segment_id)),W=M>=0;return`<div class="evcc-map-label${W?" evcc-map-label--selected":""}" style="left:${S}%;top:${g}%">
                ${W?`<span class="evcc-map-label-order">${M+1}</span>`:""}
      ...
      <div class="evcc-map-config-section">
        ...
        <div class="evcc-map-variant-row">
          ...
          <button
            ...
            ${S?"disabled":""}
          >${T}</button>
          ${h?(()=>{let M=n===p,W=a?.type==="delete"&&a?.variant===p&&a?.status==="busy",B=W?"Deleting\u2026":M?"Confirm Delete":"Delete";return`
              <button
    ...
    `}}function $t(i){let e=String(i?.floor_type??"").toLowerCase().trim(),t=String(i?.carpet_type??"").toLowerCase().trim();return e==="carpet"?t==="high_pile"||t==="high"?"carpet_high":"carpet_low":e==="carpet_low_pile"||e==="carpet_low"?"carpet_low":e==="carpet_high_pile"||e==="carpet_high"?"carpet_high":e==="hardwood"||e==="laminate"||e==="wood"?"wood":e==="tile"?"tile":e==="marble"?"marble":e==="concrete"?"concrete":e==="granite"||e==="granite_light"?"granite_light":"default"}function Ln(i){let e=String(i??""),t=2166136261;for(let c=0;c<e.length;c++)t^=e.charCodeAt(c),t=Math.imul(t,16777619)>>>0;let r=t%101,a=(t>>>13^t>>>7)%101;return`${r}% ${a}%`}function ri(i){i._resolveSegmentFloorType=function(e){return $t({floor_type:e?.floor_type??e?.floorType??"",carpet_type:e?.carpet_type??e?.carpetType??""})},i._renderFloorTextureLayer=function(e){if(!(this.card?._state?.roomFloorTextureEnabled?.()??!0))return"";let t=$t({floor_type:e?.floorType??"",carpet_type:e?.carpetType??""}),r=Oe[t]??Oe.default,a=r.opacityDefault??.85,c=`var(--evcc-floor-${t}-opacity-card,var(--evcc-floor-texture-opacity-card,${a}))`,n=Ln(e?.id??e?.name??t),s=r.layers.map(o=>{let l=`var(${o.colorToken},${o.colorDefault})`,d=`url(${o.url})`,u=`var(${o.opacityToken},${o.opacityDefault})`,m=`<span class="evcc-ftx-layer" data-role="${o.role}" style="background-color:${l};mask-image:${d};-webkit-mask-image:${d};mask-mode:luminance;-webkit-mask-source-type:luminance;--layer-opacity:${u}"></span>`;if(!o.blurToken&&!o.blurDefault)return m;let p=`var(${o.blurToken},${o.blurDefault??"0px"})`;return`<span class="evcc-ftx-blur" style="filter:blur(${p});-webkit-filter:blur(${p})">${m}</span>`}).join("");return`<div class="evcc-room-texture-layer" data-floor="${t}" style="--floor-opacity-card:${c};--floor-position-card:${n}">${s}</div>`},i._buildFloorTextureDefs=function(e){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let t=new Set,r=[];for(let a of e){if(t.has(a))continue;t.add(a);let c=St(a);c&&r.push(`<pattern id="evcc-ftx-${a}" patternUnits="userSpaceOnUse" width="8" height="8"><image href="${c}" width="8" height="8" preserveAspectRatio="xMidYMid slice"/></pattern>`)}return r.length?`<defs>${r.join("")}</defs>`:""},i._renderFloorTexturePolygon=function(e,t){if(!(this.card?._state?.mapFloorTextureEnabled?.()??!0))return"";let r=e.polygon_pct;return!Array.isArray(r)||r.length<3||!St(t)?"":`<polygon class="evcc-map-texture-polygon" points="${r.map(([c,n])=>`${c},${n}`).join(" ")}" fill="url(#evcc-ftx-${t})" data-floor="${t}"/>`}}var Pn=[{value:"hardwood",label:"Hardwood"},{value:"laminate",label:"Laminate"},{value:"tile",label:"Tile"},{value:"marble",label:"Marble"},{value:"granite",label:"Granite"},{value:"concrete",label:"Concrete"},{value:"carpet_low_pile",label:"Low-Pile Carpet"},{value:"carpet_high_pile",label:"High-Pile Carpet"}];function ii(i){i.renderSetupView=function(e){let{state:t,card:r}=e,a=r._config?.vacuum_entity_id??"",c=t.setupStatus?.()??null,n=t.setupLoading?.()??!1,s=t.setupError?.()??null,o=t.setupLastResult?.()??null,l=Array.isArray(c?.vacuums)?c.vacuums:[],d=l.find(E=>E.vacuum_entity_id===a)??null,u=Array.isArray(d?.setup_steps)&&d.setup_steps.length?d.setup_steps:Nn(d),m=d?.room_drift??null,p=t.setupRoomEditorOpenMapId?.()??null,v=t.setupRoomEditorLoadingMapId?.()??null,f=t.setupRoomEditorRooms?.()??[],h=t.setupRoomEditorSaving?.()??!1,b=t.setupDeletePendingMapId?.()??null,w=t.setupDeleteStage?.()??null,S=t.setupDeleteTypedToken?.()??"",g=t.setupDeleteDeleting?.()??!1,R=new Set((t.setupRoomEditorEnabledIds?.()??[]).map(String)),y=t.setupRoomEditorFloorTypesMap?.()??{},T=(d?.maps??[]).filter(E=>E.imported),M=n?'<div class="evcc-setup-result info">Working\u2026</div>':"",W=s&&!n?`<div class="evcc-setup-result error">${this.escapeHtml(String(s))}</div>`:"",B=(()=>{if(!o||n)return"";let E=o.status??"",V=o.message??"";return E==="error"||E==="blocked"?`<div class="evcc-setup-result error">${this.escapeHtml(V)}</div>`:V?`<div class="evcc-setup-result success">${this.escapeHtml(V)}</div>`:""})(),ne={add_vacuum:E=>E.completed?`
      ...
      `,import_active_map:E=>{let V=ai(u,"add_vacuum"),O=T.length;if(!V)return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';let U=O>0?`<div class="evcc-setup-step-body muted">${O} map${O===1?"":"s"} imported.</div>`:`<div class="evcc-setup-step-body">Import the vacuum's currently active map. Make sure it has completed a mapping run first.</div>`,Q=O>0?"Import Another Map":"Import Active Map",te=O>0?"secondary":"";return`
        ${U}
      ...
      `},save_rooms:E=>{let V=u.find(H=>H.id==="import_active_map"),O=!!V,U=!V||V.completed;if(!ai(u,"add_vacuum"))return'<div class="evcc-setup-step-body muted">Complete Add Vacuum first.</div>';if(O&&!U)return'<div class="evcc-setup-step-body muted">Complete map import first.</div>';if(T.length===0&&!O)return`
          <div class="evcc-setup-step-body">
            No rooms discovered yet. Run a clean cycle so the vacuum reports
            its room list, then refresh setup status.
          </div>
        `;let te=ue(m,d),ve=T.map(H=>be(H,!0)).join("");return`
      ...
      `}},ue=(E,V)=>{if(!E||E.in_sync)return"";let O=Array.isArray(E.new_rooms)?E.new_rooms:[],U=Array.isArray(E.removed_rooms)?E.removed_rooms:[],Q=Array.isArray(E.transiently_missing)?E.transiently_missing:[];if(O.length===0&&U.length===0&&Q.length===0)return"";let te=O.length===0?"":`
      ...
      `},fe=E=>v===E?`<div class="evcc-setup-room-editor">
        ...
        <div class="evcc-setup-room-editor">
          ...
          <div class="evcc-setup-room-list">
            ${f.length===0?'<div class="evcc-setup-step-body muted">No rooms found for this map.</div>':f.map(O=>{let U=String(O.room_id),Q=this.escapeHtml(O.name??`Room ${U}`),te=R.has(U),ve=y[U]??"hardwood",Ee=Pn.map(H=>`
              <button class="evcc-setup-floor-chip ${ve===H.value?"active":""}"
      ...
      `,ge=(E,V)=>{if(b!==E)return"";let O=this.escapeHtml(V?.typed_confirmation_value??`Map ${E}`),U=V?.requires_typed_confirmation??!1,Q=V?.reasons??[],te=Q.length?`<div class="evcc-setup-delete-badges">
           ...
           </div>`:"",ve=U?`<div class="evcc-setup-delete-typed">
      ...
      `},be=(E,V)=>{let O=String(E.map_id),U=this.escapeHtml(E.display_name??`Map ${O}`),Q=t.isSetupMapConfigured?.(O),te=p===O||v===O,ve=E.protection??null,Ee=ve?.requires_typed_confirmation??!1,H=b===O,Ac=Q&&!te?'<span class="evcc-setup-configured-badge">\u2713 Configured</span>':"",Cc=V?`
...
`);function Ot(i,e){if(!i||!e)return;let{tokens:t}=e,r=i;me.forEach(a=>{(!Object.prototype.hasOwnProperty.call(t,a.key)||t[a.key]===null||t[a.key]===void 0||t[a.key]==="")&&r.style.removeProperty(a.key)}),Object.entries(t).forEach(([a,c])=>{c!=null&&c!==""&&r.style.setProperty(a,c)})}var ec=`
  * {
    box-sizing: border-box;
    margin: 0;
    padding: 0;
