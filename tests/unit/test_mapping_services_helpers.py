...
# ---------------------------------------------------------------------------
# [MS-18] area-label anchors — READ ONLY, for the one-shot device seed
# ---------------------------------------------------------------------------
#
# The write half is gone. The m² chip's position is now stored device-locally by the
# card, in the same store the room-NAME label already used — one implementation for one
# job, where before there were two that failed differently (R2-BUG-4: neither room writer
# carried `label_anchor` through RoomConfig, so a room re-save silently dropped it).
#
# resolve_area_label_anchors SURVIVES because the card reads it ONCE per device to seed
# that local store from positions dragged before the change, so nobody's layout resets.
# _migrate_area_label_anchors is gone with the write path that was its only caller.
...
def _resolve():
    ...
    return resolve_area_label_anchors


def test_resolve_reads_the_room_record_and_the_legacy_side_table():
    """[MS-18] Both sources still feed the seed. The room record wins where both carry a
    value -- it is the newer of the two -- and a legacy entry for a room with no managed
    record is still served, which is exactly the unmanaged-room case the old migration
    deliberately refused to delete."""
    resolve = _resolve()
    bucket = {
        "rooms": {"16": {"room_id": 16, "slug": "kitchen",
                         "label_anchor": {"pct_x": 30.0, "pct_y": 80.0}}},
        "area_label_anchors": {"16": {"pct_x": 1.0, "pct_y": 2.0},
                               "99": {"pct_x": 5.0, "pct_y": 6.0}},
    }
    out = resolve(bucket)
    assert out["16"] == {"pct_x": 30.0, "pct_y": 80.0}, "the room record must win"
    assert out["99"] == {"pct_x": 5.0, "pct_y": 6.0}, "an unmanaged room's anchor was dropped"


def test_resolve_is_pure_and_never_mutates_the_bucket():
    """[MS-18c] It runs on READ paths (the dashboard snapshot + get_map_segments), so it
    must not write. A read that mutates storage is the defect RP-029/POLYGO-3 exists to
    prevent, and with the write path deleted this is the only guard left on it."""
    resolve = _resolve()
    bucket = {
        "rooms": {"16": {"room_id": 16, "slug": "kitchen"}},
        "area_label_anchors": {"16": {"pct_x": 1.0, "pct_y": 2.0}},
    ...
    resolve(bucket)
    assert bucket == snapshot, "resolve_area_label_anchors mutated the map bucket"
